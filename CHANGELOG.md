...
- 12 others were corrected from their notes.

662 lessons were updated to match their catalog entry, and the seven exploit-availability records whose value differed from their catalog entry now match it.
...
Twenty-four of the new entries include their CVE id in the title, and nine entries already in the catalog gained theirs, because CISA names a vulnerability by product and defect class and the same name then covers several records. Five weaknesses the new entries cite are now in the CWE catalog: CWE-96, CWE-131, CWE-178, CWE-497 and CWE-1289.

The Essential Eight patch gap now states what the framework requires. It was recorded as operating-system patching alone, with a description of the maturity timeframes that did not match the ISM, and entries built on it argued that the framework has no fast window or does not reach browsers, edge appliances or email gateways. It covers applications as well as operating systems, and the entry now quotes the ISM control behind each window: 48 hours from release for online services, browsers and their extensions, office suites, email clients, PDF readers, security products and the operating systems of internet-facing servers and network devices when the vendor rates the flaw critical or a working exploit exists, and the longer windows that apply otherwise. The gap it records is the one that survives: every window starts at the vendor's release, so a flaw exploited before a fix ships is open until then at any maturity level. This changes what 1,094 entries citing that control say about Australian patching obligations.

Remediation paths that would have left an operator exposed were corrected while these entries were checked. The Adobe Commerce template-injection entry now names Mage-OS, a distribution carrying the same code, and the release that fixes it, and it separates the Adobe hotfix from the encryption-key rotation that exploitation before the patch makes necessary. The Fortinet path-traversal entry no longer treats one build as closing the set of Fortinet flaws used in the same intrusions: FortiOS 6.0.10, 6.2.4 and 6.4.1 close the credential flaw, while the LDAP impersonation flaw is closed by a configuration change that the vendor documents only for 6.0.3 through 6.2.0, and a firmware upgrade does not enable it on an existing configuration. The DotNetNuke encryption entry names 9.3.1 rather than 9.2.2, whose fix was incomplete and is tracked as a separate CVE. The IBM Data Risk Manager entry states that the fixpack closing the authentication bypass is not cumulative and installs on top of the previous one.

Detection content was widened where an indicator was narrower than the exploit it claims to find. The N-able entry records the vendor's statements that the flaw was exploited in the wild alongside its release-note wording, and names the three log files to hunt in and the request shapes to filter for. The Magento entry lists the implant paths, the skimmer stored in the store's configuration table and the loader domains from the published investigation, and says which log line marks a failed include rather than a successful one.

## 0.20.0 — 2026-09-19

The minimum supported Node.js is now 24.21.0, so `engines.node` reads `>=24.21.0`. Install 24.21.0 before upgrading; npm reports an engines warning on 24.19 and 24.20. Node 24.21.0 includes OpenSSL 3.5.8 and the NSS 3.126 root certificates. Continuous integration, the release workflow, the daily-refresh and ATLAS-currency workflows, the Docker reproduction harness and `.nvmrc` all use 24.21.0, and the harness base image is pinned to the 24.21.0-alpine3.23 manifest digest.

MITRE ATLAS moves to v2026.09, which includes the v2026.08 changes. v2026.08 generalizes three techniques the catalog carries, and each entry now uses the ATLAS name and description: AML.T0072 covers any command-and-control channel rather than only a reverse shell, AML.T0075 covers enterprise resource discovery rather than only cloud services, and AML.T0089 covers enterprise environment discovery rather than only processes. v2026.09 adds eleven techniques and sub-techniques and changes none of the ones the catalog carries.

Six entries used MITRE's pre-2025 machine-learning wording and now use the current names: AML.T0007, AML.T0010, AML.T0018, AML.T0020, AML.T0024 and AML.T0044. ATLAS also widened AML.T0018 from backdooring a model to manipulating the artifact by any means, so its description was replaced rather than reworded. The skills and playbooks that print these labels next to the id now use the same names.

Two skills named the wrong ATLAS release as the one that introduced a technique. `skill-update-loop` now says ATLAS 2025.12 added AML.T0096, and `mlops-security` says ATLAS 2026.06 added AML.T0113, AML.T0114 and AML.T0091.001.

A new pre-publish check compares `data/atlas-ttps.json` against the ATLAS release it pins and fails on any id or name the release does not carry. It fetches the pinned release and caches it under `.cache/upstream/atlas/`. When it cannot reach the release, it exits non-zero. Eight names and 54 sub-technique ids in that file still disagree with the release. They are recorded as known divergences, so the check fails only on new ones, and they are tracked in issue #397.
...
Twenty-three entries now include their CVE id in the title. CISA names a vulnerability by product and defect class rather than by defect, so "Apple Multiple Products Code Execution Vulnerability" was already the title of two shipped records and would have fitted a third; where a title is shared, the identifier is appended so an operator searching the catalog no longer gets rows they cannot tell apart.

Where a listing has no vendor advisory to cite, the entry says so instead of linking something plausible. Twenty-one advisories were added across fourteen entries, each read rather than inferred — Red Hat errata, an Apache Struts bulletin, Apple's per-platform security notes, a CERT/CC vulnerability note, a vendor release note naming the fixed build. Three of them cite vendor material that describes the flaw, the affected range and the fixed version without printing the CVE number; the entry says which three.

Fourteen entries record a weakness assigned editorially because NVD published none, each naming the mechanism it is based on. Six of the fourteen use a broader classification than the rest, because the vendor publishes nothing finer — Apple describes several of these only as addressed with improved restrictions, and a narrower weakness would assert more than the record supports.

The Log4j entry's remediation no longer offers a mitigation that does nothing on the releases it is offered for. Setting the message-lookup property is an alternative only from 2.10.0 onward; on an affected release older than that the property is read by nothing, so an operator who set it and restarted was left exactly as exposed. Removing the lookup class from the deployed jar works across the whole affected range and is now listed first. The upgrade target has changed too: the entry named three follow-on defects and then told operators to install a release that closes only the first of them.

Four more records misdirected the work they describe. The Accellion FTA command-execution entry said a single build closed the whole exploited chain; it closes two of the four, and the entry now separates this defect's fixed build from the one the joint advisory directs operators to. The Apache 2.4.49 traversal entry told operators to watch for POST requests to a CGI path, while the Metasploit module for it sends a randomly generated method — a rule built from that guidance never fires on it — and every indicator on the entry now names the published exploit it was read from, down to the request paths and the response matcher. A PaperCut host running the emergency build was recorded as running a mitigation with no patch behind it, which is the state its own control reserves for a host that has no build to install; the entry now says the build is a patch and that the vendor still owes a fully tested replacement for it, rather than treating the second fact as cancelling the first. And two scoring notes explained a score their factors no longer produce, one of them still deducting for a live patch the entry had withdrawn.

Detection indicators now say where they came from. On the 62 new entries with an obtainable public exploit, each group of indicators names the template, module or advisory it was read from, with a link, and says outright which items are hunting heuristics rather than artifacts anyone observed — 66% of them cite a source that was fetched and read, against 32% across the rest of the catalog. Reading those sources changed 55 indicators and added 84. The recurring correction is an indicator that named one HTTP method or one fixed path where the published detection rule varies it, which produces a rule that never fires on the exploit it was written for: the Zimbra Autodiscover path is matched case-insensitively because the public module uses a different form, the Struts endpoint indicator no longer names a single action, and three entries now warn against keying on the method at all. Where no source could be reached, the entry says so instead of implying one.
...
The same failure closed in the pipeline collector, where it ran deeper. A workflow file too large or unreadable was skipped in silence, and every indicator it would have flipped stayed at miss. Read, parse and directory-walk failures now travel to the operator as warnings — but a warning changes no verdict on its own, so the gap reaches the indicators too: where the workflow or trust-policy inventory is incomplete, the indicators drawn from it report inconclusive rather than a clean miss, the inventory is marked uncaptured, and the readability precondition no longer attests a file the collector could not open. A repository whose one vulnerable workflow was the file that could not be read was previously reported clean.

Release workflows that resolve dependencies at build time are detected for pip, pnpm and Bundler alongside npm and cargo, so a Python or Ruby publisher no longer reads as clean. Each install command is judged on its own flags: a hash-pinned install no longer vouches for an unpinned one elsewhere in the same file, and commands chained on one line or split across lines with a trailing backslash are read as the commands they are. The requirement file is recognised whether it is attached to the short option or separated from it, and an install is not reported when the file it names is itself hash-pinned, since one hashed requirement puts pip into hash-checking mode for all of them. A bare `pnpm install` is no longer reported, because pnpm enforces the frozen lockfile whenever CI is set; only an explicit opt-out is. Bundler's job-wide frozen setting suppresses the finding only when it is switched on — `bundle config set frozen false` named the setting and disabled it, and was read as though it enabled it. A requirement file is resolved against the job's working directory as well as the repository root, so a hash-pinned file in a subdirectory is no longer reported as unpinned; where copies resolve in several directories, every one of them has to be pinned. Findings point at the line their command starts on.

Capping an upstream-catalog refresh now bounds every source. The RFC index, ATLAS and ICS-ATT&CK commands accepted the cap and discarded it, so a run asked to import a bounded batch imported the whole upstream index — and the RFC index is an order of magnitude larger than any other source the refresh reads. The cap bounds new additions only; context fields on rows already curated are still completed. The ICS-ATT&CK refresh also stops importing a cross-listed enterprise technique it cannot assign an ICS tactic to. Those rows arrived with an empty tactic and were counted as missing context by the next audit, so the import spent the gap budget on itself; they are skipped now, and the run reports how many and which.

A mistyped flag on the upstream freshness probe is refused with a non-zero exit instead of being ignored. A misspelled air-gap flag previously left the check running as though nothing had been passed, so a run believed to be offline reached the network.

Diagnostics no longer disappear when output is piped. The catalog validator's enum-mismatch and entry-count-drift messages and the skill linter's help and unknown-argument text were written to a stream the process could exit before draining, which a terminal hid and a CI log collector did not.

A malformed analyze result no longer aborts the closing phase. The evidence bundle, notification deadlines, exception language and downstream chaining are still produced, and the shape problem is reported alongside them naming the part of the result that was wrong. That holds for a single unusable entry among good ones as well as for a malformed result overall: the unusable entries are dropped and counted, the rest still reach the advisory. Catalog scoring validation likewise reports a null or non-object entry and continues through the rest of the catalog rather than stopping at the first one, and asking for the documented gaps behind an attack pattern without supplying a gap catalog returns the same answer an empty one gives instead of raising.

The manifest snapshot gate reports where it used to crash or wave through. A baseline written before a surface field existed ended the run with a stack trace on precisely the case the gate exists to report; it is now recorded as an additive change. A corrupt value where a list belongs stops the gate with the skill and field named, rather than reporting that skill's entire surface as newly added and passing.
...
Age changes what remediation means, and these entries say so rather than defaulting to patch. Several products reached end of software maintenance without a fix for the flaw CISA lists: the Cisco RV016 and RV082 have no fixed firmware while their RV042 siblings do, so an estate recording "vendor patch applied" against the model family has repaired half of it and retired none of the rest. Where that is the situation the entry names removal as the terminal state, not an upgrade path that does not exist.

A patch that is installed is not always a patch that is running. The Reader and Flash entries separate the host reboot an operator schedules from the process restart the fix actually needs: the updated code stays inert until every browser hosting the plugin and every open Reader window has been closed, which on a workstation up for days is the step that gets skipped. A remediation record keyed to the deployed version reports those hosts as done while the pre-patch code is still executing.

Where sources disagree, the entries carry the disagreement instead of picking the convenient reading. A Metasploit module places Java 5 Update 23 outside the affected set for CVE-2010-0840 while Oracle's March 2010 advisory and NVD place it inside; the entry follows the vendor and records the conflict, because an operator scoping against the looser boundary leaves the update they believed safe in range. CVE-2012-1710 carries CISA's ransomware designation and records that no public vendor reporting ties a ransomware operation to it. CVE-2012-0151 records that Microsoft reported no exploitation at release and the reporting vendor found none across its own userbase, so its in-the-wild status rests on the KEV listing alone and the entry says which evidence it has. For CVE-2015-2425, MS15-065 credits three firms with the discovery while one of them stated publicly that the flaw they found carries a different identifier — so the entry records the credit and the contest rather than asserting a finder.

Every added entry declares the framework controls it defeats across all five jurisdictions, the ATT&CK techniques it maps to, indicators a defender can hunt on, and the controls its remediation requires, drawn from the existing control set rather than by minting new ones. Those requirements now reach the framework-gap analysis: 760 CVE references land across 55 controls, 296 across 56 ATT&CK techniques. CWE-244 joins the weakness catalog to carry the Cisco appliance heap-lifetime entry.

An EPSS refresh now writes the score, the percentile and the publication date together. The refresh compared each field against its own drift threshold, so a score that moved past the threshold was written while its percentile, having moved less, kept the previous day's value — leaving an entry whose two numbers describe different publications, both in range and both plausible. The percentile is the score's rank within its publication, so the pair is only meaningful read together. The threshold now decides whether an entry refreshes rather than which of its fields does; a row missing any of the three is not a publication and is refused outright; an entry holding one half of the pair is completed from a complete row even when nothing crossed the threshold; and the prose restatement carried on those entries is regenerated with the numbers instead of describing the publication before them.

The citation scanner stops reporting a rejected CVE record when an entry simply describes code rejecting input. "Reject" is the ordinary verb for what a parser does to a malformed document or a validator does to a traversal sequence, and a mechanism description that used it was read as a claim about the CVE's record status. The record sense names what was rejected or who rejected it, and the check now requires that before flagging.
...
Two files that ship in every tarball are now hashed. npm includes `package.json` whether or not the file allowlist names it, and picks up `sources/README.md` on its own, so neither appeared in the inventory — including the one file that declares the executable entry point, the supported runtime floor and the dependency set. Both now carry SHA-256 and SHA3-512 entries and count toward the aggregate bundle digest, so a bundle verified against the SBOM covers 249 of the 268 files an operator receives rather than 247.

The SBOM says what it does not cover. The pre-computed index cache under `data/_indexes/` is deliberately excluded from per-file hashing — it is regenerated on demand, and pinning a hash would race any process that rebuilds it — but that reasoning existed only in a source comment, so an operator holding the artifact could not distinguish a partial inventory from a complete one. Two `exceptd:integrity:uncovered:*` properties now name the excluded prefix and the check that covers it instead. The publish gate additionally compares the real `npm pack` file list against the SBOM, so a file npm decides to ship that the allowlist never named fails a gate rather than shipping unhashed; the two files above are what it found.

Preparing a release no longer refreshes the test-count baseline before checking it. The refresh writes whatever count it observes, so a release that removed tests rebaselined downward and the shrinkage check — which runs later — then compared the new count against itself and passed. The check now runs first and stops the release on a drop; the refresh still follows it, so growth is still recorded.

The secret scanner's offline fallback moves to 8.30.1. It had been pinned to 8.21.2 while upstream reached 8.30.1, so an interruption to the release API would have scanned with a ruleset nine releases old and still reported success. The resolver now compares the two on every run and warns when they diverge, which is the part that was only ever a comment before.

The contributor Docker harness reports honestly. Checks that ask git what ships — line-ending coverage, tracked-file resolution — cannot answer in that image, because the repository directory is deliberately kept out of the build context so a contributor's history is never uploaded to a builder or written into an image layer. They were failing rather than saying so, which reads as a defect in the tree. They now skip with the reason stated, and the file-writing checks that were failing on directory ownership, plus the governance check that could not find files the image had excluded, work as intended. A skipped check is not a passing check: run the suite outside Docker, or read continuous integration, for the ones this image cannot cover. The ignore file previously claimed the repository directory was included when it was not, which is what made the gap look like an oversight rather than a decision.

## 0.19.2 — 2026-08-08

EPSS data on 115 catalog entries is corrected. A refresh had updated an entry's score without its percentile — and on 70 entries written a score with no percentile at all — leaving the two fields drawn from different daily publications. Because the percentile is the score's rank within its publication, the result was entries ranked in the top decile on a score that no longer supported it, which is the input exploit-prioritisation leans on hardest. Scores and percentiles are now written together. A new gate rejects any publication whose percentiles do not track its scores, requires an entry to carry both numbers or neither, and checks that the human-readable EPSS note on an entry restates the values the entry actually holds — 76 notes described an earlier publication than the fields beside them. The ordering check is a corruption detector rather than a provenance proof: it catches a stale value that contradicts the new score's rank, which is what a partial refresh produces, but two fields cannot be proven to share a publication from the catalog alone.

Technique references resolve again. ATT&CK retired `T1562` and its sub-techniques when Impair Defenses became the Defense Impairment tactic, and ATLAS 2026.07 consolidated three publish-poisoned techniques under `AML.T0115`. References to the retired identifiers survived in the D3FEND and DLP control maps, pointing operators at pages MITRE no longer serves and leaving the controls citing them mapped to nothing. Those references now name their successors. Skill text that abbreviated ATLAS runs as `AML.T0096 / T0017` now spells each identifier out, since a bare `T0017` is a different and real ICS technique. A new gate resolves every technique identifier named outside the two pinned MITRE catalogs against them, so a future pin bump cannot leave this residue behind.

`AML.T0091.001` (Use Alternate Authentication Material: Web Session Cookie), `AML.T0113` (Steal Web Session Cookie) and `AML.T0114` (AI Service Web Interface) are now carried in the ATLAS catalog; shipped skills already referenced all three.

Catalog and documentation metadata match the data again. The ATLAS catalog described its pinned release as carrying no removals or renames, which the consolidation contradicts; the ATT&CK catalog named v19.1 while pinned to v19.2; and the README, the `CONTEXT.md` inventory table and the `AGENTS.md` pin dates carried counts and dates from earlier releases. The entries re-keyed under `AML.T0115` had inherited the retired techniques' STIX identifiers and import provenance — they now carry their own, with the superseded identifier kept under a field that names it as such. The release notes for 0.18.29 listed eight new weakness classes where twelve shipped; the four omitted are CWE-138, CWE-274, CWE-441 and CWE-782.
...
KEV state across the whole catalog now reconciles against CISA's live catalog with no disagreements. Two entries CISA had listed were still recorded as unlisted — CVE-2026-45321, which also carries a known ransomware campaign, and CVE-2025-14847 — so both were scoring in the forties against a listed reality; they now score 70 and 72. Two listing dates that were off by a day or three are corrected.

Two newly listed vulnerabilities are curated in (1023 → 1025). CVE-2026-8037 is pre-authentication command injection in Progress Kemp LoadMaster, where an escape routine allocates its buffer without clearing it and never writes a terminating null, so a quote-only `apiuser` value on an unauthenticated request to `/accessv2` lets the shell argument read past its own bytes into a heap spray carried by the same request body — command execution on the appliance terminating TLS for everything behind it, rated EPSS 0.85 at the 99.7th percentile with a public detection template. CVE-2026-63077 is unauthenticated remote code execution in JetBrains TeamCity, where the agent-polling protocol deserializes attacker-controlled XML through an allowlist that kept its default permissions, dropping a JSP polyglot into the webroot and executing as the build server — a build-system takeover with credential access to everything the pipeline touches.

Live EPSS scores replace estimated ones on 59 entries.

The refresh pipeline's advisory feeds no longer force an automated pull request on every run. Those feeds report every CVE seen across Ubuntu, Red Hat, Zero Day Initiative and CISA sources that the catalog does not track — over a thousand on a typical day, and never zero, since the catalog deliberately does not carry every distribution kernel CVE. They were counted as pending catalog writes even though they cannot write the catalog, which held the "changes are pending" condition permanently true and made the daily pull request unconditional. They are now declared report-only, so that signal means what it says.

## 0.18.29 — 2026-08-06

CVE catalog currency: one hundred more CISA-KEV-listed, actively-exploited CVEs are now curated in the offline catalog (923 → 1023), covering KEV additions from August 2022 through March 2023 plus five from July and August 2026, and spanning CVE publication years 2010 through 2026. Each entry carries a verified NVD CVSS vector and CWE, ATT&CK technique mappings, real-world exploitation status, primary-source indicators where public exploit code exists, global-first framework-control-gap declarations (NIST, EU NIS2/DORA, UK CAF, AU ISM/Essential 8, ISO 27001), an RWEP score, and a paired zero-day lesson. Thirty-two carry known ransomware use.

The batch centers on the late-2022 enterprise-edge and mail-infrastructure wave — the Exchange OWASSRF chain that bypassed the ProxyNotShell URL rewrite (CVE-2022-41080, CVE-2022-41082, CVE-2022-41040), the Fortinet FortiOS SSL-VPN heap overflow behind the BOLDMOVE and COATHANGER implants (CVE-2022-42475), the Fortinet management-interface authentication bypass (CVE-2022-40684), the Citrix ADC and Gateway SAML bypass attributed to APT5 (CVE-2022-27518), the Fortra GoAnywhere MFT deserialization RCE Cl0p weaponized as a zero-day (CVE-2023-0669), the Veeam Backup & Replication unauthenticated internal-API pair used by Cuba, Conti and Yanluowang (CVE-2022-26500, CVE-2022-26501), the Zoho ManageEngine SAML RCE (CVE-2022-47966, CVE-2022-35405), the Zimbra path traversal chained through cpio (CVE-2022-41352), the SAP Internet Communication Manager request-smuggling flaw (CVE-2022-22536), the Oracle E-Business Suite and Fusion Middleware pre-auth flaws (CVE-2022-21587, CVE-2021-35587), and the Sophos Firewall code injection Sophos disclosed alongside its own targeted-exploitation notice (CVE-2022-3236).

Windows kernel and platform escalation is the other center of gravity: the CLFS driver overflows used ahead of ransomware deployment (CVE-2023-23376, CVE-2022-37969), the ALPC and COM+ Event System escalations (CVE-2023-21674, CVE-2022-41033), the CNG Key Isolation flaw reachable inside LSA (CVE-2022-41125), the Print Spooler activation-context escape (CVE-2022-41073), the JScript9 type confusion delivered through Office remote templates by APT37 (CVE-2022-41128), both Mark-of-the-Web bypasses (CVE-2022-41049, CVE-2022-41091), the Defender SmartScreen bypass (CVE-2022-44698), the Active Directory Certificate Services escalation known as Certifried (CVE-2022-26923), and the LNK shortcut flaw Stuxnet used (CVE-2010-2568). Client-side and mobile exploitation covers the Samsung clipboard-provider chain into the Mali and DPU drivers (CVE-2021-25337, CVE-2021-25369, CVE-2021-25370), Apple WebKit and kernel zero-days across iOS, iPadOS and macOS (CVE-2023-23529, CVE-2022-42856, CVE-2022-42827, CVE-2022-32917, CVE-2022-32893, CVE-2022-32894, CVE-2021-31010, CVE-2020-9934), Chromium V8, GPU, Mojo and Intents zero-days (CVE-2022-4262, CVE-2022-4135, CVE-2022-3723, CVE-2022-3075, CVE-2022-2856), and the WebRTC heap overflow Candiru used to deliver DevilsTongue (CVE-2022-2294). A long tail of Linux and Android privilege escalation (CVE-2021-3493, CVE-2013-6282, CVE-2013-2596, CVE-2013-2094, CVE-2011-1823, CVE-2013-2597), signed-driver escalation primitives suitable for bring-your-own-vulnerable-driver (CVE-2018-19320 through CVE-2018-19323, CVE-2015-2291), and internet-facing devices recruited into botnets (CVE-2018-6530, CVE-2022-26258, CVE-2011-4723, CVE-2017-5521, CVE-2018-7445) rounds out the set.

Twelve weakness classes backing these entries are new to the CWE catalog: CWE-138 (improper neutralization of special elements), CWE-197 (numeric truncation), CWE-259 (hard-coded password), CWE-270 (privilege context switching error), CWE-274 (improper handling of insufficient privileges), CWE-311 (missing encryption of sensitive data), CWE-406 (network amplification), CWE-441 (unintended proxy or intermediary, the confused-deputy class), CWE-664 (improper control of a resource through its lifetime), CWE-782 (exposed IOCTL with insufficient access control), CWE-824 (access of uninitialized pointer), and CWE-1285 (improper validation of a specified index, position or offset). CVE-2013-2094 now maps to CWE-1285 rather than the CWE-189 category, which MITRE prohibits using to map real-world vulnerabilities.

## 0.18.28 — 2026-08-05

`doctor` no longer reports a healthy install as failed once a skill passes ninety days since its last threat review. The skill-currency check treated every skill outside the "current" tier as an error, while the currency model itself raises `action_required` only below the "acceptable" tier — so a maintained skill crossing the ninety-day mark turned the whole health check red and `doctor --json` exited 1, on a fixed cadence, with nothing actually wrong. Review-date drift is now advisory: the check reports `severity: "warn"`, `all_green` stays false so the drift is still visible, and the exit code stays 0. A skill genuinely past its review window still fails with exit 1, and a critically stale one is still counted critical. The check reports `drifting_skills` and `action_required_skills` alongside the existing `stale_skills` list, which keeps its meaning, so a consumer can distinguish an advisory state from a failure. Human output distinguishes the three states rather than printing "all green" under a warning icon.

The `global-grc` skill carries a refreshed threat review. The EU AI Act entry now follows the application timeline as amended by Regulation (EU) 2026/1744: Art. 50 transparency obligations and Commission supervision of GPAI providers are in force from 2026-08-02, the high-risk requirements including Art. 9 risk management and Art. 15 cybersecurity are deferred to 2027-12-02 for Annex III systems and 2028-08-02 for Annex I product-embedded systems, and two new Art. 5 prohibitions covering non-consensual intimate imagery and child sexual abuse material apply from 2026-12-02. Art. 73's serious-incident clocks are corrected — 2 days for widespread infringement or critical-infrastructure disruption, 10 days for death of a person, 15 days otherwise — having previously been transposed. The compliance-theater test that keyed off the superseded August 2026 deadline is replaced by one testing Art. 50 transparency, which is enforceable now, and one testing high-risk classification against the deferred dates, on the ground that a deferral is not a withdrawal while the attack surface stays live. NIS2 no longer claims enforcement across all 27 Member States: transposition remains incomplete, and four Member States were referred to the CJEU in July 2026, so entities scope by Member State of establishment rather than by the directive text. CRA manufacturer reporting under Art. 14 is now covered in the notification and exploit-availability tables — 24-hour early warning, 72-hour notification, 14-day final report from 2026-09-11 — a clock triggered by active exploitation and therefore driven directly by KEV state. UK coverage is corrected: NCSC CAF v4.0 does carry AI-relevant content in Principle B4's automated-decision-making outcomes and its ETSI Securing AI reference, so the claim that no UK framework addressed AI is withdrawn and replaced with the narrower residual gap, and the Cyber Security and Resilience Bill is recorded as not yet in force. Canada gains the Critical Cyber Systems Protection Act enacted by Bill C-8, with its 72-hour incident report to the Communications Security Establishment, and the lapsed Bill C-27 is replaced by Bill C-36. Australia gains the Enhanced Critical Infrastructure Risk Management Program Rules and their supply-chain mapping obligation, the most specific such requirement in the registry. Japan's active-cyber-defence legislation is corrected to the 2025 Acts and the 2026 APPI amendment is recorded. CSA CCM is updated to v4.1.

The `security-maturity-tiers` skill carries a refreshed threat review. Its MVP tier adds an inventory step for self-hosted LLM gateways and flow builders, a product class that has produced repeated KEV-listed pre-authentication remote code execution against default deployments, and the tier's completion criterion now requires that none be reachable from the internet. The US federal remediation baseline is updated: CISA BOD 26-04 replaced BOD 22-01's uniform KEV due dates with risk-tiered ones, so the framework-lag table records a 3-day top band against the tier's hours-scale live-patch prescription. EU CRA dates are corrected — Annex I product requirements apply from 2027-12-11, while Art. 14 reporting starts 2026-09-11 — and the EU AI Act Art. 15 row reflects the deferral, with the tier's rationale sharpened: the obligation does not bind for another 16 to 24 months while the attack surface is live today. Post-quantum TLS now cites RFC 9954 and notes that X25519MLKEM768 is offered by default from OpenSSL 3.5 onward, making the Practical-tier step a version upgrade and a config assertion rather than a migration project.

RFC 9954 (Hybrid Key Exchange in TLS 1.3, Informational, July 2026) is now carried in the offline RFC catalog, so `exceptd rfc 9954` resolves without network access instead of reporting an unknown reference. Its predecessor `draft-ietf-tls-hybrid-design` records the transition rather than still describing itself as queued for a number, and `pqc-first` no longer describes the hybrid construction as a draft — while noting that `draft-ietf-tls-ecdhe-mlkem`, the Standards-Track profile naming the concrete groups, remains one, so the construction reaching RFC status does not standardise the named groups.

Both skills gain a Defensive Countermeasure Mapping section routing each finding to a D3FEND technique and the downstream skill that owns its implementation, and both now declare the D3FEND catalog and the techniques they cite so the mappings reach the index and cross-reference surfaces rather than reporting zero.

## 0.18.27 — 2026-07-30

CVE catalog currency: one hundred more CISA-KEV-listed, actively-exploited CVEs are now curated in the offline catalog (823 → 923), covering additions to the CISA Known Exploited Vulnerabilities catalog from March through August 2023 (plus four July 2026 additions) and spanning CVE publication years 2004 through 2026. Each entry carries a verified NVD CVSS vector and CWE, ATT&CK technique mappings, real-world exploitation status, primary-source indicators where public exploit code exists, global-first framework-control-gap declarations (NIST, EU NIS2/DORA, UK CAF, AU ISM/Essential 8, ISO 27001), an RWEP score, and a paired zero-day lesson. The batch centers on the 2023 mass-exploitation wave against file-transfer, email-security, and enterprise-edge infrastructure — the Progress MOVEit Transfer SQL-injection abused by Cl0p in the largest data-theft campaign of the year (CVE-2023-34362), the Barracuda Email Security Gateway command injection exploited by UNC4841 (CVE-2023-2868), the PaperCut MF/NG authentication-bypass RCE used by Cl0p and LockBit affiliates (CVE-2023-27350), the Adobe ColdFusion access-control and deserialization chain (CVE-2023-26359, CVE-2023-26360, CVE-2023-29298, CVE-2023-38205), the Ivanti EPMM/MobileIron authentication bypass and Sentry chain (CVE-2023-35078, CVE-2023-35081, CVE-2023-38035), the Citrix ShareFile storage-zones RCE (CVE-2023-24489), the Oracle WebLogic deserialization RCE (CVE-2023-21839), the Zyxel firewall command injection recruited into Mirai (CVE-2023-28771), and the VMware Aria/Tools flaws tied to UNC3886 (CVE-2023-20867, CVE-2023-20887). It also covers client and mobile exploitation heavily used by commercial-spyware and state actors — the Apple "Operation Triangulation" kernel flaw and the June-2023 iOS WebKit/kernel zero-day chain (CVE-2023-38606, CVE-2023-37450, CVE-2023-32409, CVE-2023-32434, CVE-2023-32435, CVE-2023-32439, CVE-2023-28204, CVE-2023-32373), the Microsoft Outlook NTLM-relay privilege escalation used by APT28/Forest Blizzard (CVE-2023-23397), the Microsoft Office RCE deployed by Storm-0978/RomCom (CVE-2023-36884) and the Windows SmartScreen bypass used by Magniber (CVE-2023-24880), the Samsung mobile-chipset chain surfaced in spyware exploitation (CVE-2021-25487, CVE-2021-25489, CVE-2021-25394, CVE-2021-25395, CVE-2021-25371, CVE-2021-25372), the Arm Mali GPU kernel driver flaws chained in Android spyware (CVE-2022-22706, CVE-2022-38181, CVE-2021-29256, CVE-2023-26083), and multiple Google Chrome V8 and Skia zero-days (CVE-2023-2033, CVE-2023-3079, CVE-2023-2136). A long tail of internet-facing devices recruited into botnets rounds out the set — TP-Link Archer (CVE-2023-1389), Ruckus wireless (CVE-2023-25717), D-Link routers (CVE-2019-17621, CVE-2019-20500), the end-of-life Zyxel P660HN (CVE-2017-18368), SolarView Compact (CVE-2022-29303) — alongside the Ignite Realtime Openfire path traversal mined by Kinsing (CVE-2023-32315), the Veeam Backup & Replication credential disclosure (CVE-2023-27532), the Samba "SambaCry" RCE (CVE-2017-7494), the Apache Log4j 2.15 follow-up (CVE-2021-45046), and older but still-exploited Linux privilege escalations and Cisco IOS flaws. Two weakness classes backing these entries are new to the CWE catalog: CWE-294 (authentication bypass by capture-replay, the mechanism behind the Outlook NTLM-relay flaw) and CWE-401 (missing release of memory after effective lifetime).

## 0.18.26 — 2026-07-22

Every CVE in the offline catalog now declares a framework-control-gap statement for all five jurisdiction buckets — NIST, EU (NIS2/DORA), UK (CAF), AU (Essential 8/ISM), and ISO 27001:2022. Previously two-thirds of the catalog mapped only a subset, so operators subject to multiple regimes reading the framework-gap output got US- or EU-centric coverage on most entries. The remaining partial entries have been completed with vulnerability-specific gap statements — each tied to that CVE's own exploitation profile (exploitation window, attack vector, end-of-life status, blast radius) and resolving to a real control in the framework-control-gap catalog — bringing the whole catalog to full multi-jurisdiction coverage. A new predeploy gate enforces this, so a newly-curated CVE can no longer ship with a partial jurisdiction set.

## 0.18.25 — 2026-07-22

CVE catalog currency: four more CISA-KEV-listed, actively-exploited CVEs are now curated in the offline catalog (819 → 823), covering CISA Known Exploited Vulnerabilities catalog additions dated 2026-07-21. The set is the WordPress Core "wp2shell" pre-authentication remote-code-execution chain — CVE-2026-60137 (SQL injection in the WP_Query author__not_in parameter) plus CVE-2026-63030 (a REST batch-endpoint interpretation conflict that desynchronizes handler dispatch) — which reaches default installations of WordPress 6.9.x and 7.0.x and carries an accelerated three-day KEV deadline; CVE-2026-0770, an unauthenticated code-execution flaw in Langflow's /api/v1/validate/code endpoint (exec() over attacker-supplied Python), for which GitHub's advisory records no fixed release, so the effective control is removing endpoint exposure and network-isolating the service rather than patching; and CVE-2021-27137, a DD-WRT UPnP/SSDP stack buffer overflow patched in 2021 but newly weaponized in 2026 by the Gafgyt/C0XMO botnet to recruit routers as DDoS bots. Each entry carries a verified CVSS vector and CWE, ATT&CK technique mappings, real-world exploitation status, primary-source indicators, global-first framework-control-gap declarations across NIST, EU NIS2, UK CAF, AU Essential 8, and ISO 27001, an RWEP score, and a paired zero-day lesson.

## 0.18.24 — 2026-07-21

CVE catalog currency: one hundred more CISA-KEV-listed, actively-exploited CVEs are now curated in the offline catalog (719 → 819), covering additions to the CISA Known Exploited Vulnerabilities catalog from August 2023 through March 2024 and spanning CVE publication years 2014 through 2024. Each entry carries a verified NVD CVSS vector and CWE, ATT&CK technique mappings, real-world exploitation status, primary-source indicators where public exploit code exists, global-first framework-control-gap declarations (NIST, EU NIS2/DORA, UK CAF, AU ISM/Essential 8, ISO 27001), an RWEP score, and a paired zero-day lesson. The batch is weighted toward internet-facing edge and enterprise infrastructure — the Citrix NetScaler "CitrixBleed" session-token disclosure mass-exploited by ransomware affiliates (CVE-2023-4966), the JetBrains TeamCity authentication bypass (CVE-2024-27198), the Atlassian Confluence broken-access-control and template-injection chain abused by Cerber ransomware (CVE-2023-22515, CVE-2023-22518, CVE-2023-22527), the Cisco IOS XE Web UI privilege-escalation-to-implant chain that compromised tens of thousands of devices (CVE-2023-20198, CVE-2023-20273), the F5 BIG-IP Configuration utility authentication-bypass-to-RCE pair (CVE-2023-46747, CVE-2023-46748), the SysAid path traversal used by Lace Tempest (CVE-2023-47246), the Ivanti EPMM/MobileIron authentication bypass (CVE-2023-35082), the Juniper Junos OS J-Web PHP external-variable-modification chain (CVE-2023-36844), and the Unitronics PLC insecure-default-credential flaw exploited against water utilities (CVE-2023-6448) — alongside client and mobile exploitation including the WinRAR file-name-spoofing code execution (CVE-2023-38831), the Windows SmartScreen and Internet Shortcut security-feature bypasses chained to deliver stealer and RAT payloads (CVE-2023-36025, CVE-2024-21412), the Apple "Operation Triangulation" zero-click WebKit and kernel chain (CVE-2023-41990, CVE-2023-42916, CVE-2023-42917), multiple Google Chrome/V8 zero-days (CVE-2023-4762, CVE-2023-6345, CVE-2024-0519), and the Microsoft Windows kernel privilege escalation used by Lazarus (CVE-2024-21338). Five weakness classes backing these entries are new to the CWE catalog: CWE-295 (improper certificate validation), CWE-351 (insufficient type distinction), CWE-473 (PHP external variable modification), CWE-703 (improper check or handling of exceptional conditions), and CWE-823 (use of out-of-range pointer offset).

## 0.18.23 — 2026-07-18

CVE catalog currency: one hundred more CISA-KEV-listed, actively-exploited CVEs are now curated in the offline catalog (619 → 719), covering additions to the CISA Known Exploited Vulnerabilities catalog from March 2024 through July 2026 and spanning CVE publication years 2008 through 2026. Each entry carries a verified NVD CVSS vector and CWE, ATT&CK technique mappings, real-world exploitation status, primary-source indicators where public exploit code exists, global-first framework-control-gap declarations (NIST, EU NIS2/DORA, UK CAF, AU ISM/Essential 8, ISO 27001), an RWEP score, and a paired zero-day lesson. The batch is weighted toward internet-facing edge and enterprise infrastructure — the Palo Alto PAN-OS GlobalProtect command injection exploited as a zero-day (CVE-2024-3400), the PHP-CGI argument-injection RCE mass-exploited by TellYouThePass ransomware (CVE-2024-4577), a Linux kernel nf_tables use-after-free local privilege escalation (CVE-2024-1086), the Fortinet FortiClientEMS SQL-injection RCE (CVE-2023-48788), the Cisco ASA/FTD pair used in the ArcaneDoor campaign (CVE-2024-20353, CVE-2024-20359), the Check Point Security Gateway information disclosure (CVE-2024-24919), a VMware ESXi authentication bypass abused by ransomware affiliates (CVE-2024-37085), the OSGeo GeoServer evaluation RCE (CVE-2024-36401), the ServiceNow Now Platform input-validation RCE chain (CVE-2024-4879, CVE-2024-5217), and recent Microsoft SharePoint (CVE-2026-58644, CVE-2026-56164), Active Directory Federation Services (CVE-2026-56155), Oracle E-Business Suite (CVE-2026-46817), Fortinet FortiSandbox (CVE-2026-25089, CVE-2026-39808), and SonicWall SMA1000 (CVE-2026-15409, CVE-2026-15410) additions — alongside client and browser exploitation including six Google Chrome/V8 flaws (CVE-2024-4671, CVE-2024-4761, CVE-2024-4947, CVE-2024-5274, CVE-2024-7965, CVE-2024-7971), the Windows MSHTML spoofing flaw used by Void Banshee to deliver the Atlantida stealer (CVE-2024-43461), an Android Pixel firmware privilege escalation (CVE-2024-32896), and the Justice AV Solutions Viewer supply-chain backdoor (CVE-2024-4978). Fourteen weakness classes backing these entries are new to the CWE catalog: CWE-27, CWE-80, CWE-184, CWE-191, CWE-203, CWE-276, CWE-280, CWE-358, CWE-415, CWE-591, CWE-645, CWE-670, CWE-783, and CWE-1336.

## 0.18.22 — 2026-07-11

CVE catalog currency: one hundred more CISA-KEV-listed, actively-exploited CVEs are now curated in the offline catalog (519 → 619), covering the most recent additions to the CISA Known Exploited Vulnerabilities catalog (September 2024 through July 2026). Each entry carries a verified NVD CVSS vector and CWE, ATT&CK technique mappings, real-world exploitation status, primary-source indicators where public exploit code exists, global-first framework-control-gap declarations (NIST, EU NIS2/DORA, UK CAF, AU ISM/Essential 8, ISO 27001), an RWEP score, and a paired zero-day lesson. The batch spans internet-facing edge and enterprise infrastructure — Palo Alto PAN-OS management auth bypass (CVE-2025-0108), SonicWall SonicOS SSLVPN authentication bypass (CVE-2024-53704), Fortinet FortiManager "FortiJump" (CVE-2024-47575) and the FortiOS fgfmd format-string RCE (CVE-2024-23113), SimpleHelp path traversal on RMM servers (CVE-2024-57727), an Ivanti Cloud Services Appliance path-traversal chain (CVE-2024-8963), Veeam Backup & Replication deserialization RCE (CVE-2024-40711), Trimble Cityworks deserialization (CVE-2025-0994), and Microsoft SharePoint code injection (CVE-2024-38094) — alongside client and mobile exploitation such as the Apple iOS USB Restricted Mode bypass used in targeted attacks (CVE-2025-24200), a Firefox animation-timeline use-after-free deployed by RomCom (CVE-2024-9680), the Windows NTLM hash-disclosure flaw (CVE-2024-43451), and an Android Framework privilege escalation (CVE-2024-43093). Two of the hundred were AI-assisted discoveries, including the PTZOptics camera command-injection pair surfaced by an LLM triaging honeypot traffic (CVE-2024-8956, CVE-2024-8957); the Langflow server-side code execution flaw is also included (CVE-2026-55255). Six weakness classes backing these entries are new to the CWE catalog: CWE-114, CWE-134, CWE-176, CWE-273, CWE-303, and CWE-425.

Batch curation tooling: the new `exceptd refresh --curate-batch` command promotes a set of KEV drafts into fully-curated catalog entries from a facts file and a per-CVE judgment file in one pass, refusing to write any entry whose CWE, ATT&CK, or framework-control references are not already present in their catalogs. The nightly data refresh now pre-enriches newly-KEV-listed CVEs with NVD (CVSS, CWE) and FIRST EPSS data at import time and raises the per-run discovery cap from twenty to one hundred, so auto-imported drafts arrive with their mechanical fields already populated; `vendor_advisories` now reflects only real vendor advisories rather than a placeholder, so the catalog-gap detector correctly surfaces entries still missing one.

## 0.18.21 — 2026-07-03

CVE catalog currency: twenty more CISA-KEV-listed, actively-exploited CVEs are now curated in the offline catalog (519 total), each carrying a verified NVD CVSS vector and CWE, an ATT&CK technique mapping, behavioral indicators, global-first framework-control-gap declarations, an RWEP score, and a paired zero-day lesson. The additions cover Microsoft SharePoint Server deserialization (CVE-2026-45659) and the Windows MSC "EvilTwin" console-file bypass exploited by EncryptHub/Water Gamayun (CVE-2025-26633); the SimpleHelp OIDC signature-verification bypass on internet-facing RMM servers (CVE-2026-48558); the three Ivanti Endpoint Manager credential-coercion path traversals (CVE-2024-13159, CVE-2024-13160, CVE-2024-13161); the Advantive VeraCore upload and SQL-injection pair used by XE Group (CVE-2024-57968, CVE-2025-25181); a Linux kernel HID information leak used in a Cellebrite Android-unlock chain (CVE-2024-50302); Progress WhatsUp Gold path-traversal RCE (CVE-2024-4885); a Windows Win32k local privilege escalation (CVE-2018-8639); the Hitachi Vantara Pentaho BA Server authorization-bypass-plus-injection chain (CVE-2022-43769, CVE-2022-43939); the end-of-life Cisco Small Business RV command injection abused by the PolarEdge botnet (CVE-2023-20118); Zimbra Collaboration Suite stored XSS (CVE-2023-34192); Microsoft Partner Center (CVE-2024-49035) and Power Pages (CVE-2025-24989) access-control flaws; Oracle Agile PLM (CVE-2024-20953) and Adobe ColdFusion BlazeDS (CVE-2017-3066) deserialization RCE; and the Palo Alto PAN-OS authenticated arbitrary file read that attackers chain with the CVE-2025-0108 auth bypass (CVE-2025-0111). Four weakness classes backing these entries are new to the CWE catalog: CWE-707, CWE-404, CWE-647, and CWE-610.

Threat-intelligence pins: MITRE ATLAS advances from v2026.05 to v2026.06 across the manifest, the TTP catalog metadata, and every operator-facing reference. The update is additive — v2026.06 adds three techniques (Steal Web Session Cookie, its web-session-cookie sub-technique, and AI Service Web Interface) and refreshes the LLM Jailbreak technique, with no technique-ID removals or renames, so every cited AML.T* mapping stays valid.
