...
The release orchestrator's code-scanning gate asks the repository-wide question. It queried alerts against the pull request's merge reference, which reports only what that pull request introduced — so an alert already present on the main branch returned nothing and the phase reported a clean result. The gate now queries the repository, prints the reference each alert sits on, and is filtered to the analysis tool whose findings it is meant to block, so policy alerts from other tools cannot make the list permanently non-empty and therefore useless.
