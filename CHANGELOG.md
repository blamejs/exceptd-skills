...
CVE catalog — Langflow unauthenticated RCE, and a same-CVSS exploitation-signal contrast. Adds two unauthenticated remote-code-execution flaws in Langflow, the visual LLM app/agent builder, where a flow endpoint reaches a code-execution path without authentication. **CVE-2025-3248** (CWE-94 / CWE-306, VulnCheck CNA CVSS 9.8 CRITICAL) — the `/api/v1/validate/code` endpoint runs attacker-supplied Python with no authentication; it is **listed in the CISA KEV catalog** (added 2025-05-05) and actively exploited. **CVE-2026-33017** (CWE-94 / CWE-95 / CWE-306, NVD CVSS 9.8; GitHub CNA CVSS v4.0 9.3) — after the first fix, the public flow-build endpoint still ran flow-supplied Python through an unsandboxed dynamic-evaluation path; fixed in 1.9.0, not KEV-listed. Both carry identical CVSS 9.8, but RWEP ranks the KEV-listed, actively-exploited entry P1 (patch within 24h) and the patched, non-KEV sibling far lower — exploitation signal, not severity, drives priority. They introduce NEW-CTRL-103: every LLM-app-builder flow validate/build/run endpoint must authenticate and sandbox submitted code, and the fix must cover the whole class of endpoints rather than the single reported route. Upgrade Langflow to 1.9.0 or later. CVE count 383 → 384.
