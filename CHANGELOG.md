...
The contributor Docker harness reproduces continuous integration again. Repository metadata never reached the build context, so every check that asks git what ships — line-ending coverage, ignored-file resolution, the diff against the base — could not answer inside the image; the working directory was owned by root while its contents were not, which git refuses to work across; and the image both omitted files the governance check requires and included a maintainer's untracked local notes, which the version-stamp check then counted as shipped surface. Gates now report the same result in the harness as on a runner.
