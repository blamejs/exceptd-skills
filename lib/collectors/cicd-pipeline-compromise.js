...
function collect({ cwd = process.cwd(), env = process.env, args = {} } = {}) {
  ...
  return {
    // Attest the two playbook preconditions:
    //   ci-config-readable — the collector just walked workflow YAML
    //     + OIDC trust JSON; filesystem reads succeeded.
    //   operator-owns-ci-fleet — the operator invoked the collector
    //     against their own cwd; running `exceptd collect` on a repo
    //     is the act of attesting ownership of that repo's CI.
    precondition_checks: {
      "cwd-is-repo": true,
      "ci-config-readable": true,
      "operator-owns-ci-fleet": true,
    },
    artifacts,
    signal_overrides,
    collector_meta: {
