...
function scanOidcPolicies(root) {
  ...
  function walk(dir, depth) {
    ...
    for (const e of entries) {
      ...
      if (!text) continue;
      // Cheap literal pre-filter: skip files that never mention the GitHub OIDC
      // issuer at all. A lookalike (`…githubusercontent.com.evil`) that slips
      // past this substring check is still rejected by the authoritative
      // sub-claim patterns below — each is self-anchored by the leading `"` of
      // the JSON key (`"token.actions.githubusercontent.com:sub"`), so a spoofed
      // `"eviltoken.actions…"` key cannot match.
      if (!text.includes("token.actions.githubusercontent.com")) continue;
