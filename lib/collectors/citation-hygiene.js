...
function recordRejectedOrDisputed(note, selfId) {
  ...
  while ((m = re.exec(note)) !== null) {
    ...
    // (a) Another CVE id adjacent on either side → the status is about THAT
    //     record, not this citation.
    const adjacent = `${before} ${after}`.match(/CVE-\d{4}-\d{4,}/gi) || [];
    if (adjacent.some((c) => c.toUpperCase() !== self)) continue;
    // (b) A 'dispute(d)' qualified by a non-record noun is a disagreement about
