...
function scanDockerfile(content, rel) {
  ...
  // Build-stage aliases declared via `FROM <image> AS <alias>`. A later
  // `FROM <alias> AS <next>` is an INTERNAL multi-stage reference, not a
  // registry image — it needs no tag/digest. Without tracking these, every
  // `FROM build-stage` in a normal multi-stage Dockerfile false-fired
  // from-latest / no-digest-pin.
  const stageAliases = new Set();
  // ARG defaults declared earlier in the file, used to resolve interpolated
  // base references like `FROM node:${NODE_VERSION}` or `FROM ${BASE_IMAGE}`.
  // A digest pinned through an ARG default (`ARG BASE=img@sha256:...` then
  // `FROM ${BASE}`) is a legitimate pinning pattern that was invisible to the
  // raw-text check; and an unresolvable interpolation can't be proven to be
  // `:latest`, so flagging it from-latest was a false positive.
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Track `ARG NAME=default` (and bare `ARG NAME`) so a later FROM that
    // interpolates the name can resolve it. Quotes around the default are
    // stripped so `ARG BASE="img@sha256:..."` resolves cleanly.
    ...
    if (fromMatch) {
      ...
      // Skip `scratch` (special base) AND references to a previously-declared
      // build stage (internal multi-stage reference — not a registry image, so
      // a tag/digest is neither present nor required).
      if (ref !== "scratch" && !stageAliases.has(ref.toLowerCase())) {
        const hasDigest = /@sha256:[0-9a-f]{64}/i.test(ref);
        // When an interpolation could not be resolved, the actual image
        // reference is unknown — it may carry a digest supplied via
        // `--build-arg` at build time. Suppress both signals rather than
        // emit a guess; a resolvable ARG default still produces a precise
        // verdict below.
...
// Shared with `discover`: the relative paths of container artifacts
// (Dockerfiles / Containerfiles / compose files, including variants like
// docker-compose.test.yml or examples/wiki/Dockerfile) anywhere in the bounded
// tree this playbook walks — so `discover` recommends `containers` for exactly
// the surface the collector scans, not just a root-level exact-name Dockerfile
// or docker-compose.yml. Filename-only (no content reads), capped, and
// fault-tolerant so a walk error can't break discovery.
...
module.exports = { playbook_id: COLLECTOR_ID, collect, classify, hasContainerArtifacts };
