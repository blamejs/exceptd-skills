...
function scanDockerfile(content, rel) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (userMatch) {
      sawAnyUser = true;
      const u = userMatch[1];
      // Numeric 0, "root", or empty count as root.
      if (u !== "0" && u !== "root" && !/^root:/.test(u)) sawNonRootUser = true;
      continue;
    }
...
function scanCompose(content, rel) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^\s*network_mode:\s*['"]?host\b/i.test(line)) hits["compose-host-network"].push({ file: rel, line: i + 1, snippet: line.trim() });
    // cap_add: SYS_ADMIN — either inline list `cap_add: [SYS_ADMIN]`
    // or multi-line `cap_add:\n  - SYS_ADMIN`.
    if (/cap_add:.*SYS_ADMIN/i.test(line) ||
        (/^\s*-\s*['"]?(SYS_ADMIN|CAP_SYS_ADMIN)\b/i.test(line) &&
...
function scanK8s(content, rel) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^\s*runAsUser:\s*0\b/.test(line)) hits["k8s-run-as-root"].push({ file: rel, line: i + 1, snippet: line.trim() });
