...
function collect({ cwd = process.cwd(), env = process.env, args = {} } = {}) {
  ...
  return {
    precondition_checks: {
      ...
      // gate from what we walked. The runner's autoDetectPreconditions probes
      // the run process's cwd (not the collected --cwd) and has no
      // exists_any() branch, so a repo that clearly has a source tree would
      // otherwise surface a spurious precondition_unverified warning. Keying
      // to the gate's exact id mirrors the sbom / library-author collectors;
      // `repo-context` is not a precondition this playbook references.
      "repo-has-source-tree": sourceFiles.length > 0,
    },
    artifacts,
    signal_overrides,
