...
// Token vocabulary that signals a SECURITY-CRITICAL use of the hash
...
// remaining vocabulary still catches crypto-security uses via
// `hash`/`hmac`/`sign`/`signature`/`token`/`jwt`/`verify`/`password`.
const WEAK_HASH_VAR_FLOW_RE = /(hmac|sign|signature|token|jwt|verify|password|hash[-_]?(?:credential|secret|password|key|auth))/i;
// Filename-path demotion: paths that explicitly indicate
// content-addressable / fingerprint / etag / cache-key concerns get
// demoted because the var-flow regex alone can't distinguish those
// from auth contexts when the filename's intent is clear.
...
function scanWeakHash(content, rel) {
  ...
  // Filename-path demotion: paths like `integrity.go` / `hashing.go`
  // / `fingerprint.py` are content-addressable use, not auth.
