...
// des-cbc. Both fields absent → undefined (inconclusive).
function parseWeakMacOrCipher(macs, ciphers) {
  ...
  const cipherWeak = ciphers && /(?:^|,)(?:arcfour(?:128|256)?|3des-cbc|des-cbc|blowfish-cbc)(?:,|$)/.test(ciphers);
...
// shell-interpolated. ENOENT / EACCES / non-zero exit → null;
// caller surfaces that via collector_errors / unflipped indicators.
function readOrSpawn(pathOverride, cmd, args, errors) {
  ...
  try {
    ...
    if (errors) errors.push({ artifact_id: cmd, kind: "spawn_failed", reason: `${cmd} ${args.join(" ")}: ${e.code || e.message}` });
    return null;
  }
}
