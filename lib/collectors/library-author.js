...
function scanPublishWorkflow(content, rel) {
  ...
  // static-token: workflow references secrets.NPM_TOKEN /
  // secrets.PYPI_API_TOKEN / similar without a corresponding
  // `permissions: id-token: write`.
  const usesStaticToken = /\bsecrets\.(NPM_TOKEN|PYPI_API_TOKEN|PYPI_TOKEN|CARGO_REGISTRY_TOKEN|MAVEN_TOKEN|GH_TOKEN)\b/.test(content);
...
function collect({ cwd = process.cwd(), env = process.env, args = {} } = {}) {
  ...
  // package.json provenance opt-in.
  ...
  if (pkgManifest && pkgManifest.content) {
    try {
      ...
      const provenance = j?.publishConfig?.provenance;
      provenanceMissing = provenance === true ? "miss" : "hit";
    } catch (e) {
      errors.push({ artifact_id: "package-manifest", kind: "parse_failed", reason: `package.json: ${e.message}` });
    }
  }

  // lockfile-missing-integrity: same predicate as the sbom
  // collector's lockfile-no-integrity, but published under THIS
  // playbook's indicator id (library-author uses
  // `lockfile-missing-integrity`, sbom uses `lockfile-no-integrity`).
  let lockfileMissingIntegrity = "miss";
  const npmLock = path.join(root, "package-lock.json");
  if (fs.existsSync(npmLock)) {
    try {
      const j = JSON.parse(fs.readFileSync(npmLock, "utf8"));
      let withoutIntegrity = 0;
      const walkObj = (obj) => {
        if (!obj || typeof obj !== "object") return;
        if (obj.resolved && obj.integrity == null) withoutIntegrity++;
        for (const v of Object.values(obj)) if (v && typeof v === "object") walkObj(v);
      };
      walkObj(j.packages || j.dependencies || {});
      if (withoutIntegrity > 0) lockfileMissingIntegrity = "hit";
    } catch {
      // Malformed lockfile — leave unflipped.
    }
  }
  ...
  const signal_overrides = {
    ...
    "lockfile-missing-integrity": lockfileMissingIntegrity,
