...
function looksLikePublishWorkflow(name, content) {
  // Heuristic: filename starts with `release`/`publish`/`deploy`/`build`,
  // OR the workflow body invokes a known publish/release tool (npm /
  ...
  if (/^(release|publish|deploy|build)/.test(lower)) return true;
