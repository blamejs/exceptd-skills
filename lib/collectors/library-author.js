...
// sigstore keyless signing. It is valid at workflow-level permissions,
// at a specific job's `permissions:` block, or even inline — so any
// occurrence of the `id-token: write` token anywhere in a workflow file
// counts as the capability being present. `idTokenWriteAvailable` lets
// the caller pass a repo-wide view (the token may be declared in a
// reusable / called workflow distinct from the file being scanned).
function hasIdTokenWriteAnyScope(content) {
  return /\bid-token:\s*write\b/.test(content);
}

function scanPublishWorkflow(content, rel, idTokenWriteAvailable) {
  const hits = {
  ...
  // OIDC is available when this file declares `id-token: write` at any
  // scope (workflow or job), OR a sibling workflow in the repo does
  // (reusable-workflow / split publish setups).
  const hasIdTokenWrite = idTokenWriteAvailable || hasIdTokenWriteAnyScope(content);
...
function collect({ cwd = process.cwd(), env = process.env, args = {} } = {}) {
  ...
  // `id-token: write` may be declared in a job other than the one that
  // publishes, or in a reusable workflow the publish workflow calls.
  // Treat the OIDC capability as present if ANY workflow file in the
  // repo declares it at any scope.
  const idTokenWriteAvailable = workflows.some(w => hasIdTokenWriteAnyScope(w.content));
  for (const w of publishWorkflows) {
    const h = scanPublishWorkflow(w.content, w.rel, idTokenWriteAvailable);
