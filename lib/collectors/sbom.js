...
function findSbomDocuments(cwd) {
  ...
  for (const s of SBOM_FORMATS) {
    ...
    try {
      ...
      let content;
      // Read from the already-open descriptor (no re-open by path → no
      // TOCTOU window, preserving the fstat-on-fd hardening). readFileSync(fd)
      // loops internally to EOF; a single readSync(fd, buf, …, stat.size) does
