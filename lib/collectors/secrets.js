...
function carriesPrivateKey(file) {
  ...
  try {
    ...
    // Read the whole descriptor to EOF. A single readSync may return fewer
    // bytes than requested (a short read on a network / FUSE / interrupted fd),
    // which would leave the buffer's tail NUL-padded; if the BEGIN marker sat
    // past that boundary — e.g. a .pem with a leading `Bag Attributes` /
    // subject= header block, the default OpenSSL pkcs12→PEM layout — the marker
    // test would return false and silently drop a real private key. readFileSync
    // on the open fd loops internally until EOF, so the test always sees the
    // complete content. Reading from the already-open fd keeps the no-TOCTOU
    // open→fstat ordering (no second path lookup).
