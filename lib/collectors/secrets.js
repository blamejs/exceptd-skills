...
function collect({ cwd = process.cwd(), env = process.env, args = {} } = {}) {
  ...
  signal_overrides["world-writable-env-file"] = worldWritablePosture.some(p => p.world_writable) ? "hit" : "miss";
  signal_overrides["ssh-key-bad-perms"] = sshPrivateKeys.some(f => {
    const p = statPosture(f.full);
    return p.world_readable;
  }) ? "hit" : "miss";
