...
function gapReport(frameworkIds, threatScenario, controlGaps, cveCatalog = {}, opts = {}) {
  ...
  const newControls = (lessonEntry && Array.isArray(lessonEntry[1].new_control_requirements))
    ? lessonEntry[1].new_control_requirements
      ...
      // than omitting it, so require the identifying field before mapping.
      // The data itself is fixed; the guard is here because operator output
      // should degrade to silence rather than to garbage, and the catalog test
      // fails on the malformed shape so this cannot hide one.
