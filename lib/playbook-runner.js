...
function validate(playbookId, directiveId, analyzeResult, agentSignals = {}, runOpts = {}) {
  ...
  // Fallback when no path's preconditions are verified (the common static-scan
  // case): prefer the highest-priority path that actually ADDRESSES a fired
  // indicator, so the recommendation matches the finding rather than always
  // surfacing priority-1. Falls back to priority-1 when no fired-signal-linked
  // path exists (or no path declares for_signals) — fully backward-compatible.
  if (!selected && paths.length) {
    selected = paths.find(addressesFired) || paths[0];
  }
