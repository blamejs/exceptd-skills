  ...
  if (format === 'csaf-2.0') {
    ...
    const cveVulns = analyze.matched_cves.map(c => {
      ...
      const base = {
        ...
        remediations,
        // Bind this CVE's per-version CSAFPID leaves (from the product tree)
        // alongside the synthetic target product, so a CSAF consumer correlating
        // product_status back to the branches tree resolves the real affected
        // versions, not only the opaque synthetic product.
        product_status: isFixed
          ? { fixed: [productId, ...(csafProductTree.cveProductIds[c.cve_id] || [])] }
