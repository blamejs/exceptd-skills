...
function close(playbookId, directiveId, analyzeResult, validateResult, agentSignals = {}, runOpts = {}) {
  ...
  const enrichNotification = (na) => {
    ...
    const clockStart = obligation
      ? computeClockStart(obligation.clock_starts, agentSignals, runOpts, engineClassification, phaseFlags)
      : null;
 ...
 */
function computeClockStart(eventName, agentSignals, runOpts = {}, engineClassification = null, phaseFlags = {}) {
  ...
  // detect_confirmed.
  if (eventName === 'detect_confirmed' && detected) return new Date();
  if (eventName === 'analyze_complete' && phaseFlags && phaseFlags.analyze_complete === true) return new Date();
  if (eventName === 'validate_complete' && phaseFlags && phaseFlags.validate_complete === true) return new Date();
  return null;
}
