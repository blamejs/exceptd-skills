...
function analyze(playbookId, directiveId, detectResult, agentSignals = {}, runOpts = {}) {
  ...
  // attribute to gate on; in that case factor scaling treats catalog
  // attributes as absent (factor=0 except blast_radius which is independent).
  const factorCve = matchedCves[0] || null;
  ...
  for (const input of an.rwep_inputs || []) {
    ...
    const scale = _factorScale(input.rwep_factor, factorCve, blastRadiusScore);
