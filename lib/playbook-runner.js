...
function run(playbookId, directiveId, agentSubmission = {}, runOpts = {}) {
  ...
  try {
    ...
    // phases.validate.verdict + phases.analyze.rwep.adjusted to find
    // the answer; the most common ops question ("did this playbook
    // detect anything?") needs a one-line answer at the top.
    const verdict = (phases.validate && phases.validate.verdict) || 'inconclusive';
