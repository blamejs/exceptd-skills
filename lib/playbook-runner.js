...
function lockDir() {
  ...
  // tampering with this user's lock set in the shared tmpdir.
...
function analyze(playbookId, directiveId, detectResult, agentSignals = {}, runOpts = {}) {
  ...
  for (const ec of an.escalation_criteria || []) {
    if (evalCondition(ec.condition, { ...agentSignals, ...evalCtxRoot, rwep: adjustedRwep, blast_radius_score: blastRadiusScore, theater_verdict: theaterVerdict, compliance_theater_check: result.compliance_theater_check, jurisdiction_obligations: (playbook.phases && playbook.phases.govern && playbook.phases.govern.jurisdiction_obligations) || [], analyze: result, finding: findingShape }, playbook)) {
...
function evalCondition(expr, ctx, playbook) {
  ...
  if (quant) {
    ...
    const cmp = inner.match(/^([A-Za-z_][\w-]*)((?:\.[A-Za-z_][\w-]*)+)\s*(>=|<=|==|=|<|>|!=)\s*(.+)$/);
    if (cmp) {
      const head = cmp[1];
      const arr = resolvePath(ctx, head);
      if (Array.isArray(arr)) {
        // `any head.rest op rhs` → does any element of head satisfy `<element>.rest op rhs`?
        // Re-root the predicate at each element by evaluating against a synthetic ctx.
        const predicate = `_qitem${cmp[2]} ${cmp[3]} ${cmp[4]}`;
        const test = el => evalCondition(predicate, { ...ctx, _qitem: el }, playbook);
  ...
  // satisfied when ANY field value equals the member — so `contains 'EU'`
  // matches the obligation whose jurisdiction is EU. A delimiter-/shape-specific
  // matcher silently disabled every one of these clauses.
  ...
  if (m) {
    ...
    return arr.some((el) =>
      el === member ||
      (el && typeof el === 'object' && Object.values(el).includes(member))
    );
  }
  ...
  if (m) {
    const members = m[2].split(',').map((s) => s.trim().replace(/^['"]|['"]$/g, '')).filter((s) => s.length);
    const lv = resolvePath(ctx, m[1]);
    if (Array.isArray(lv)) return lv.some((x) => members.includes(String(x)));
    return lv != null && members.includes(String(lv));
  }
