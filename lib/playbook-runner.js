...
function run(playbookId, directiveId, agentSubmission = {}, runOpts = {}) {
  ...
  try {
    ...
    if (phases.analyze && Array.isArray(phases.analyze.matched_cves) && phases.analyze.matched_cves.length) {
      ...
      // Prefer a deterministic or high-confidence hit; fall back to the first
      // hit. Leaves topFinding null when nothing fired, so summary_line does
      // not echo the verdict.
