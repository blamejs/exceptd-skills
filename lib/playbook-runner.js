...
function analyze(playbookId, directiveId, detectResult, agentSignals = {}, runOpts = {}) {
  ...
  // exploitation / KEV / PoC multipliers (baseRwep already excludes it). Fall
  // back to matchedCves[0] only when no eligible CVE exists, in which case
  // baseRwep is 0 and the scaling is harmless.
  let factorCve = rwepEligible[0] || matchedCves[0] || null;
  if (factorCve) {
