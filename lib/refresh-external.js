...
function epssTripleDiffs(id, local, fetched, drift) {
  ...
  const out = [];
  if (score != null && score !== local.epss_score)
    out.push({ id, field: "epss_score", before: local.epss_score ?? null, after: score, severity: "medium" });
  if (percentile != null && percentile !== local.epss_percentile)
    out.push({ id, field: "epss_percentile", before: local.epss_percentile ?? null, after: percentile, severity: "medium" });
  if (date && date !== local.epss_date)
    out.push({ id, field: "epss_date", before: local.epss_date ?? null, after: date, severity: "low" });
  return out;
}
