    ...
    for (const r of report.results) {
      ...
      for (const d of r.discrepancies || []) {
        if (d.field === "cisa_kev" || d.field === "cisa_kev_date") {
          ...
          // (true→false) of an entry carrying strong human-curated exploitation
          // signal is far more likely a transient/incomplete feed than a genuine
          // CISA removal, so surface it for review (applyDiff skips review_only)
          // instead of auto-downgrading the curated entry. Without this the live
          // path silently de-listed confirmed-exploitation CVEs the cache path
          // would have held back.
          if (d.field === "cisa_kev" && d.local === true && d.fetched === false &&
              hasCuratedExploitSignal(ctx.cveCatalog && ctx.cveCatalog[r.cve_id])) {
            diff.review_only = true;
            diff.note = `KEV de-listing held for review: ${r.cve_id} carries curated exploitation signal; confirm a genuine CISA removal before downgrading.`;
          }
          diffs.push(diff);
        }
      }
