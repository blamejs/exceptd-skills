...
// Cap the registry metadata read so a hostile/misconfigured endpoint can't
// stream unbounded JSON into RAM (the /latest response is a few KB; 16 MB is
// generous headroom). Symmetric with getBuffer's tarball streaming cap.
const JSON_RESPONSE_CAP_BYTES = 16 * 1024 * 1024;
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.get({
      ...
      if (res.statusCode !== 200) {
        ...
        e.statusCode = res.statusCode; // let withRetry's classifier retry 429/5xx
        return reject(e);
      }
      const chunks = [];
      let total = 0;
      res.on("data", (c) => {
        total += c.length;
        if (total > JSON_RESPONSE_CAP_BYTES) {
          req.destroy(new Error(`registry response exceeds ${JSON_RESPONSE_CAP_BYTES}-byte cap`));
          return;
        }
        chunks.push(c);
      });
    ...
    req.on("timeout", () => { const te = new Error("timeout"); te.code = "ETIMEDOUT"; req.destroy(te); });
    req.on("error", reject);
  });
}

// Adopt the vendored blamejs retry primitive: a transient registry failure
// (HTTP 429/5xx, ECONNRESET/ETIMEDOUT family) is retried with exponential
// backoff + jitter instead of aborting the whole --network refresh on the
// first network hiccup. Permanent conditions (parse error, size-cap) carry no
// statusCode/code, so the classifier does not retry them.
function getJson(url, timeoutMs) {
  // Lazy require: a parseTar-only consumer (scripts/verify-shipped-tarball.js)
  // loads this module in a context where vendor/ may not be present, and it
  // never calls getJson — so retry must not be a load-time dependency.
  const { withRetry } = require("../vendor/blamejs/retry.js");
  return withRetry(() => getJsonOnce(url, timeoutMs), { maxAttempts: 3, baseDelayMs: 100, maxDelayMs: 2000, jitterFactor: 0.5 });
}

// The tarball URL is taken from the registry metadata (or, under
// EXCEPTD_REGISTRY_FIXTURE, a local fixture file), so it is not a hardcoded
// constant the way the /latest metadata URL is. Pin its host to the npm
// registry the metadata was queried from, so a tampered metadata response or
// fixture cannot steer the fetch at an internal or attacker-controlled address.
// Content trust still rests on the SHA-512/SRI + shasum + Ed25519 checks below;
// this is fetch-destination defense-in-depth.
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.get({
      ...
      if (res.statusCode !== 200) {
        res.resume();
        const e = new Error(`HTTP ${res.statusCode} from ${url}`);
        e.statusCode = res.statusCode; // retryable 429/5xx via withRetry
        return reject(e);
      }
      const chunks = [];
      let total = 0;
...
// Wrap the tarball fetch in the same vendored retry primitive (BJS-01). The
// host-allowlist rejection + size-cap carry no statusCode/code, so a permanent
// refusal is not retried; a transient 5xx / reset / timeout is.
function getBuffer(url, timeoutMs) {
  const { withRetry } = require("../vendor/blamejs/retry.js"); // lazy — see getJson
  return withRetry(() => getBufferOnce(url, timeoutMs), { maxAttempts: 3, baseDelayMs: 100, maxDelayMs: 2000, jitterFactor: 0.5 });
}
...
// Pinned npm registry signing keys (https://registry.npmjs.org/-/npm/v1/keys).
// The registry signs `<pkg>@<version>:<integrity>` with an ECDSA P-256
// (secp256r1 / ecdsa-sha2-nistp256) key; `dist.signatures[]` carries the
// keyid + base64 signature. These keys are pinned in-tree on purpose: fetching
// them over the network would mean authenticating the metadata response with a
// key obtained from the same untrusted channel, which is circular. They also
// rotate rarely and out-of-band. Values are SPKI DER, base64-encoded.
const NPM_REGISTRY_KEYS = Object.freeze({
...
// Verify the npm registry's signature over `<pkg>@<version>:<integrity>`.
// Returns:
//   { status: "valid", keyid }            — a pinned key verified the signature
//   { status: "absent" }                  — no dist.signatures entries supplied
//   { status: "unverifiable", reason }     — integrity missing (nothing to sign over)
//   { status: "unknown-keyid", keyid }     — signature keyid is not a pinned npm key
//   { status: "invalid", reason }          — a present, pinned-keyid signature did NOT verify
//
// The caller treats `valid` as an authenticity anchor over the WHOLE tarball
// (including the unsigned data/ tree), `invalid`/`unknown-keyid` as a hard
// refusal, and `absent`/`unverifiable` as warn-and-continue (so private or
// pre-signing registries still work — the SHA-512/SHA-1 gates still apply).
function verifyNpmRegistrySignature(pkg, version, integrity, signatures) {
  if (!Array.isArray(signatures) || signatures.length === 0) return { status: "absent" };
  if (typeof integrity !== "string" || integrity.length === 0) {
    return { status: "unverifiable", reason: "dist.integrity missing — no canonical message to verify the registry signature over" };
  }
  const message = Buffer.from(`${pkg}@${version}:${integrity}`, "utf8");
  for (const entry of signatures) {
    if (!entry || typeof entry.keyid !== "string" || typeof entry.sig !== "string") continue;
    const spkiB64 = NPM_REGISTRY_KEYS[entry.keyid];
    if (!spkiB64) return { status: "unknown-keyid", keyid: entry.keyid };
    let keyObj;
    try {
      keyObj = crypto.createPublicKey({ key: Buffer.from(spkiB64, "base64"), format: "der", type: "spki" });
    } catch (e) {
      return { status: "invalid", reason: `pinned npm key ${entry.keyid} failed to load: ${e.message}` };
    }
    let ok = false;
    try { ok = crypto.verify("sha256", message, keyObj, Buffer.from(entry.sig, "base64")); }
    catch (e) { return { status: "invalid", reason: `crypto.verify threw for keyid ${entry.keyid}: ${e.message}` }; }
    if (ok) return { status: "valid", keyid: entry.keyid };
  }
  return { status: "invalid", reason: "no dist.signatures[] entry verified against the pinned npm registry key" };
}
...
async function main() {
  ...
  try {
    if (process.env.EXCEPTD_REGISTRY_FIXTURE) {
      ...
      meta = await getJson(`https://registry.npmjs.org/${encodeURIComponent(PKG_NAME).replace(/%40/g, "@").replace(/%2F/g, "/")}/latest`, opts.timeoutMs);
    }
  } catch (e) {
  ...
  if (!SEMVER_RE.test(String(latestVersion))) {
    emit({ ok: false, error: `registry metadata version is not valid semver: ${String(latestVersion).slice(0, 64)}` }, opts.json);
    process.exitCode = 2; return;
  }
  const canonicalUrl = canonicalTarballUrl(latestVersion);
  ...
  // Verify SHA-512 SRI first (collision-resistant beyond SHA-1 reach), then
  // SHA-1 shasum for compatibility. Both are transport hashes drawn from the
  // registry metadata response, so they only prove the downloaded tarball
  // matches the hashes THAT RESPONSE declared — an attacker who can forge the
  // metadata serves a tampered tarball plus matching SHA-512/SHA-1 and both
  // pass. The npm registry's signature over `<pkg>@<version>:<integrity>`
  // (verified further below, after the both-absent refusal) is the only layer
  // an attacker forging the metadata cannot also forge, because it is made
  // with npm's registry private key, which they do not hold.
  //
  // `integrityVerified` records whether at least one transport hash actually
  // matched the downloaded bytes. The per-skill + manifest Ed25519 signatures
  // below authenticate skills/ + manifest.json, but the swap loop ALSO
  // replaces data/ and manifest-snapshot.json, which carry no per-content
  // signature — a transport hash is their only integrity anchor. When the
  // registry metadata supplies NEITHER dist.integrity (sha512 SRI) NOR a
  // dist.shasum, both branches are skipped and those bytes would otherwise be
  // swapped in with zero verification, so the both-absent case is refused
  // below rather than falling through to an unauthenticated swap.
  ...
  if (tarballIntegrity && /^sha512-/.test(tarballIntegrity)) {
    ...
    // below still gates. A non-sha512 SRI is not actually checked here, so it
    // does not count toward integrityVerified.
  ...
  // Refuse the swap when no transport hash was verifiable. data/ and
  // manifest-snapshot.json have no per-content signature, so absent both
  // dist.integrity and dist.shasum there is no integrity anchor on those bytes
  // and a tampered registry response (or a hostile EXCEPTD_REGISTRY_FIXTURE)
  // could inject arbitrary catalog/playbook/index content. `npm update -g`
  // runs the full provenance-verified path (sha512 SRI + npm Ed25519 registry
  // signatures + sigstore attestations), so it is the correct fallback.
  if (!integrityVerified) {
    emit({
      ok: false,
      error: "registry metadata carries no verifiable transport integrity (dist.integrity sha512 SRI and dist.shasum both absent) — refusing to swap unsigned data/ + manifest-snapshot.json",
      hint: "data/ and manifest-snapshot.json are not covered by the per-skill or manifest Ed25519 signature; a transport hash is their only integrity anchor. Run `npm update -g @blamejs/exceptd-skills` for the full provenance-verified path.",
    }, opts.json);
    process.exitCode = 4; return;
  }

  // Verify the npm registry's signature over `<pkg>@<version>:<integrity>`.
  // This is the authenticity anchor the transport hashes above cannot be: the
  // hashes come from the same (possibly forged) metadata response as the
  // tarball URL, but this signature is made with npm's registry private key.
  // It transitively authenticates the WHOLE tarball — including the data/ and
  // manifest-snapshot.json bytes that carry no per-content Ed25519 signature.
  //   - valid           → strongest anchor present; proceed.
  //   - invalid / unknown-keyid → a present signature did not verify against a
  //                       pinned npm key (or steers us to an unpinned key);
  //                       this is the forged-metadata signal — refuse.
  //   - absent / unverifiable   → registry supplied no signature (private or
  //                       pre-signing registry); the SHA-512/SHA-1 gates above
  //                       still applied, so warn-and-continue.
  const regSig = verifyNpmRegistrySignature(PKG_NAME, latestVersion, tarballIntegrity, registrySignatures);
  if (regSig.status === "invalid" || regSig.status === "unknown-keyid") {
    emit({
      ok: false,
      error: `npm registry signature ${regSig.status} for ${PKG_NAME}@${latestVersion} — refusing to swap`,
      reason: regSig.reason || null,
      keyid: regSig.keyid || null,
      hint: "dist.signatures is present but did not verify against the pinned npm registry key. A forged metadata response cannot reproduce this signature, so treat it as tampering. Run `npm update -g @blamejs/exceptd-skills` for the full provenance-verified path, or report at https://github.com/blamejs/exceptd-skills/issues.",
    }, opts.json);
    process.exitCode = 5; return;
  }
  if (regSig.status === "absent" || regSig.status === "unverifiable") {
    progress(`note: npm registry signature ${regSig.status} (${regSig.reason || "no dist.signatures"}) — relying on transport-hash integrity only`, opts.json);
  }
  ...
  // The swap loop replaces `data/` + `manifest.json` + `manifest-snapshot.json`
  // in addition to `skills/`. None of those files are covered by the per-skill
  // Ed25519 signature (which signs only the skill body bytes). Their integrity
  // anchors are: the SHA-512 SRI + SHA-1 shasum transport hashes (checked
  // above; both-absent is refused), AND — when the registry supplies it — the
  // npm registry ECDSA-P256 signature over `<pkg>@<version>:<integrity>`
  // (verifyNpmRegistrySignature above), which authenticates the whole tarball
  // against npm's pinned signing key and cannot be reproduced by a forged
  // metadata response. `npm update -g` additionally honors dist.attestations
  // (sigstore SLSA provenance), which this data-only path does not consume.
  //
  ...
  try {
    ...
    emit({
      ...
      registry_signature_status: regSig.status,
      registry_signature_keyid: regSig.keyid || null,
      message: `refreshed catalog from v${localVersion} → v${latestVersion} (${verifiedCount}/${skills.length} skill signatures verified, npm registry signature: ${regSig.status}). Backup at ${path.relative(ROOT, backupDir)} — safe to remove after verifying the new run.`,
    }, opts.json);
  } catch (e) {
...
module.exports = {
  ...
  // Exported for in-process tests of the tarball fetch-destination allowlist.
  isAllowedTarballHost,
  // Exported for in-process tests of the npm registry signature verification
  // over `<pkg>@<version>:<integrity>`.
