#!/usr/bin/env node
"use strict";

/**
 * `exceptd refresh --network` — fetch the npm-published tarball, verify every
 * skill signature against the local install's Ed25519 key, and swap in the
 * fresh `data/`, `manifest.json` and `manifest-snapshot.json`. The local
 * `keys/public.pem` is the trust anchor; --network never rotates it.
 */

const fs = require("fs");
const path = require("path");
const https = require("https");
const crypto = require("crypto");
const zlib = require("zlib");
const os = require("os");

const ROOT = path.resolve(__dirname, "..");
const PKG_NAME = "@blamejs/exceptd-skills";
const PKG_UNSCOPED = PKG_NAME.includes("/") ? PKG_NAME.split("/").pop() : PKG_NAME;
// Strict semver — the only registry-metadata value embedded in the fetch URL.
const SEMVER_RE = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/;

// Built from literals plus the guarded version, never from the metadata's
// dist.tarball, so a tampered /latest response cannot steer the download.
function canonicalTarballUrl(version) {
  return `https://registry.npmjs.org/${PKG_NAME}/-/${PKG_UNSCOPED}-${version}.tgz`;
}
const REQUEST_TIMEOUT_MS = 15000;

function parseArgs(argv) {
  const out = { force: false, dryRun: false, timeoutMs: REQUEST_TIMEOUT_MS, json: false, airGap: false };
  for (let i = 2; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--force") out.force = true;
    else if (a === "--dry-run") out.dryRun = true;
    else if (a === "--json") out.json = true;
    else if (a === "--air-gap") out.airGap = true;
    else if (a === "--timeout") out.timeoutMs = parseInt(argv[++i], 10) || REQUEST_TIMEOUT_MS;
  }
  if (process.env.EXCEPTD_AIR_GAP === "1") out.airGap = true;
  return out;
}

function emit(obj, json) {
  if (json) process.stdout.write(JSON.stringify(obj) + "\n");
  else if (obj.ok) process.stdout.write(`[refresh-network] ${obj.message || JSON.stringify(obj)}\n`);
  else process.stderr.write(`[refresh-network] FAIL: ${obj.error || JSON.stringify(obj)}\n`);
}

function progress(line, json) {
  // Progress goes to stderr even in plain mode, so stdout carries only the result.
  process.stderr.write(`[refresh-network] ${line}\n`);
}

// Caps the registry metadata read against an endpoint streaming unbounded JSON.
const JSON_RESPONSE_CAP_BYTES = 16 * 1024 * 1024;

function getJsonOnce(url, timeoutMs) {
  return new Promise((resolve, reject) => {
    const u = new URL(url);
    const req = https.get({
      host: u.host, path: u.pathname + u.search,
      headers: { "Accept": "application/json", "User-Agent": "exceptd/refresh-network" },
      timeout: timeoutMs,
    }, (res) => {
      if (res.statusCode !== 200) {
        res.resume();
        const e = new Error(`HTTP ${res.statusCode} from ${url}`);
        e.statusCode = res.statusCode; // let withRetry's classifier retry 429/5xx
        return reject(e);
      }
      const chunks = [];
      let total = 0;
      res.on("data", (c) => {
        total += c.length;
        if (total > JSON_RESPONSE_CAP_BYTES) {
          req.destroy(new Error(`registry response exceeds ${JSON_RESPONSE_CAP_BYTES}-byte cap`));
          return;
        }
        chunks.push(c);
      });
      res.on("end", () => {
        try { resolve(JSON.parse(Buffer.concat(chunks).toString("utf8"))); }
        catch (e) { reject(new Error(`parse: ${e.message}`)); }
      });
    });
    req.on("timeout", () => { const te = new Error("timeout"); te.code = "ETIMEDOUT"; req.destroy(te); });
    req.on("error", reject);
  });
}

// Transient failures (429/5xx, ECONNRESET/ETIMEDOUT) retry; permanent ones
// (parse error, size cap) carry no statusCode/code, so the classifier skips them.
function getJson(url, timeoutMs) {
  // Lazy: verify-shipped-tarball.js loads this module for parseTar where vendor/ may be absent.
  const { withRetry } = require("../vendor/blamejs/retry.js");
  return withRetry(() => getJsonOnce(url, timeoutMs), { maxAttempts: 3, baseDelayMs: 100, maxDelayMs: 2000, jitterFactor: 0.5 });
}

const ALLOWED_TARBALL_HOST = /(?:^|\.)npmjs\.org$|(?:^|\.)npmjs\.com$/;

// Rejects a non-default port before the hostname test: `registry.npmjs.org:9999`
// passes a hostname-only allowlist yet connects to the attacker's port.
function isAllowedTarballHost(url) {
  try {
    const u = new URL(url);
    if (!(u.port === "" || u.port === "443")) return false;
    return ALLOWED_TARBALL_HOST.test(u.hostname.toLowerCase());
  } catch { return false; }
}

function getBufferOnce(url, timeoutMs) {
  return new Promise((resolve, reject) => {
    const u = new URL(url);
    if (!isAllowedTarballHost(url)) {
      return reject(new Error(`refusing to fetch tarball from non-allowlisted host: ${u.hostname}`));
    }
    const cap = (() => {
      const env = parseInt(process.env.EXCEPTD_TARBALL_SIZE_CAP_BYTES, 10);
      return Number.isFinite(env) && env > 0 ? env : 200 * 1024 * 1024;
    })();
    const req = https.get({
      host: u.host, path: u.pathname + u.search,
      headers: { "User-Agent": "exceptd/refresh-network" },
      timeout: timeoutMs,
    }, (res) => {
      if (res.statusCode !== 200) {
        res.resume();
        const e = new Error(`HTTP ${res.statusCode} from ${url}`);
        e.statusCode = res.statusCode; // retryable 429/5xx via withRetry
        return reject(e);
      }
      const chunks = [];
      let total = 0;
      res.on("data", (c) => {
        total += c.length;
        if (total > cap) {
          req.destroy(new Error(`tarball exceeds ${cap}-byte cap during streaming download`));
          return;
        }
        chunks.push(c);
      });
      res.on("end", () => resolve(Buffer.concat(chunks)));
    });
    req.on("timeout", () => { const te = new Error("timeout"); te.code = "ETIMEDOUT"; req.destroy(te); });
    req.on("error", reject);
  });
}

// Same classifier as getJson: the host refusal and the size cap are permanent.
function getBuffer(url, timeoutMs) {
  const { withRetry } = require("../vendor/blamejs/retry.js"); // lazy — see getJson
  return withRetry(() => getBufferOnce(url, timeoutMs), { maxAttempts: 3, baseDelayMs: 100, maxDelayMs: 2000, jitterFactor: 0.5 });
}

/**
 * Parse an uncompressed tar buffer into [{ name, body }] — the GNU ustar
 * variant npm produces; a type-L entry supplies the following entry's name.
 */
function parseTar(buf) {
  const entries = [];
  let offset = 0;
  let pendingLongName = null;
  // An entry whose name escapes the extraction root is dropped.
  const isSafeName = (n) => {
    if (typeof n !== "string" || n.length === 0) return false;
    if (/^[\\/]/.test(n) || /^[A-Za-z]:[\\/]/.test(n)) return false;
    return !n.split(/[\\/]/).some((seg) => seg === "..");
  };
  while (offset + 512 <= buf.length) {
    const block = buf.subarray(offset, offset + 512);
    // An all-zero block is the end-of-archive marker.
    if (block.every((b) => b === 0)) break;
    let name = block.subarray(0, 100).toString("utf8").replace(/\0.*$/, "");
    const sizeStr = block.subarray(124, 136).toString("utf8").replace(/\0.*$|\s+$/g, "").trim();
    const size = parseInt(sizeStr, 8) || 0;
    const type = String.fromCharCode(block[156] || 0);
    const prefix = block.subarray(345, 500).toString("utf8").replace(/\0.*$/, "");
    if (prefix) name = prefix + "/" + name;
    if (pendingLongName) { name = pendingLongName; pendingLongName = null; }
    const dataStart = offset + 512;
    const dataEnd = dataStart + size;
    if (type === "L") {
      pendingLongName = buf.subarray(dataStart, dataEnd).toString("utf8").replace(/\0.*$/, "");
    } else if (type === "0" || type === "" || type === "\0") {
      if (isSafeName(name)) {
        entries.push({ name, body: buf.subarray(dataStart, dataEnd) });
      }
    }
    offset = dataStart + Math.ceil(size / 512) * 512;
  }
  return entries;
}

function fingerprintPublicKey(pemText) {
  try {
    const ko = crypto.createPublicKey(pemText);
    const der = ko.export({ type: "spki", format: "der" });
    return crypto.createHash("sha256").update(der).digest("base64");
  } catch { return null; }
}

function verifyDetached(publicKeyObj, payload, sigB64) {
  try {
    return crypto.verify(null, payload, publicKeyObj, Buffer.from(sigB64, "base64"));
  } catch { return false; }
}

// Pinned npm registry signing keys (https://registry.npmjs.org/-/npm/v1/keys),
// as base64 SPKI DER. Pinned in-tree because fetching them over the channel
// they authenticate would be circular.
const NPM_REGISTRY_KEYS = Object.freeze({
  "SHA256:jl3bwswu80PjjokCgh0o2w5c2U4LhQAE57gj9cz1kzA":
    "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAE1Olb3zMAFFxXKHiIkQO5cJ3Yhl5i6UPp+IhuteBJbuHcA5UogKo0EWtlWwW6KSaKoTNEYL7JlCQiVnkhBktUgg==",
  "SHA256:DhQ8wR5APBvFHLF/+Tc+AYvPOdTpcIDqOhxsBHRwC7U":
    "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEY6Ya7W++7aUPzvMTrezH6Ycx3c+HOKYCcNGybJZSCJq/fd7Qa8uuAKtdIkUQtQiEKERhAmE5lMMJhP8OkDOa2g==",
});

// Returns { status }: valid | absent | unverifiable | unknown-keyid | invalid.
// Callers refuse on invalid and unknown-keyid, and warn-and-continue otherwise.
function verifyNpmRegistrySignature(pkg, version, integrity, signatures) {
  if (!Array.isArray(signatures) || signatures.length === 0) return { status: "absent" };
  if (typeof integrity !== "string" || integrity.length === 0) {
    return { status: "unverifiable", reason: "dist.integrity missing — no canonical message to verify the registry signature over" };
  }
  const message = Buffer.from(`${pkg}@${version}:${integrity}`, "utf8");
  for (const entry of signatures) {
    if (!entry || typeof entry.keyid !== "string" || typeof entry.sig !== "string") continue;
    const spkiB64 = NPM_REGISTRY_KEYS[entry.keyid];
    if (!spkiB64) return { status: "unknown-keyid", keyid: entry.keyid };
    let keyObj;
    try {
      keyObj = crypto.createPublicKey({ key: Buffer.from(spkiB64, "base64"), format: "der", type: "spki" });
    } catch (e) {
      return { status: "invalid", reason: `pinned npm key ${entry.keyid} failed to load: ${e.message}` };
    }
    let ok = false;
    try { ok = crypto.verify("sha256", message, keyObj, Buffer.from(entry.sig, "base64")); }
    catch (e) { return { status: "invalid", reason: `crypto.verify threw for keyid ${entry.keyid}: ${e.message}` }; }
    if (ok) return { status: "valid", keyid: entry.keyid };
  }
  return { status: "invalid", reason: "no dist.signatures[] entry verified against the pinned npm registry key" };
}

// CRLF/BOM normalization, duplicated to keep this module dependency-free. Any
// change must be mirrored in lib/verify.js, lib/sign.js and
// scripts/verify-shipped-tarball.js; tests/normalize-contract.test.js enforces it.
function normalizeSkillBytes(buf) {
  let s = Buffer.isBuffer(buf) ? buf.toString("utf8") : String(buf);
  if (s.length > 0 && s.charCodeAt(0) === 0xFEFF) s = s.slice(1);
  return Buffer.from(s.replace(/\r\n/g, "\n"), "utf8");
}

// Canonical-bytes computation must stay in lockstep with
// canonicalManifestBytes() in lib/sign.js and lib/verify.js.
function canonicalizeForRefresh(value) {
  if (Array.isArray(value)) return value.map(canonicalizeForRefresh);
  if (value && typeof value === "object") {
    const out = {};
    for (const key of Object.keys(value).sort()) {
      out[key] = canonicalizeForRefresh(value[key]);
    }
    return out;
  }
  return value;
}
function canonicalManifestBytesForRefresh(manifest) {
  const clone = Object.assign({}, manifest);
  delete clone.manifest_signature;
  const json = JSON.stringify(canonicalizeForRefresh(clone), null, 2);
  return normalizeSkillBytes(Buffer.from(json, "utf8"));
}
function verifyTarballManifestSignature(manifest, publicKeyPem) {
  const sig = manifest && manifest.manifest_signature;
  if (!sig || typeof sig !== "object") return { status: "missing" };
  if (typeof sig.signature_base64 !== "string") {
    return { status: "invalid", reason: "manifest_signature.signature_base64 missing or not a string" };
  }
  if (sig.algorithm !== "Ed25519") {
    return { status: "invalid", reason: `manifest_signature.algorithm must be 'Ed25519' (got ${JSON.stringify(sig.algorithm)})` };
  }
  let signatureBytes;
  try { signatureBytes = Buffer.from(sig.signature_base64, "base64"); }
  catch (e) { return { status: "invalid", reason: `malformed base64: ${e.message}` }; }
  const bytes = canonicalManifestBytesForRefresh(manifest);
  let ok = false;
  try {
    ok = crypto.verify(null, bytes, {
      key: publicKeyPem,
      dsaEncoding: "ieee-p1363",
    }, signatureBytes);
  } catch (e) {
    return { status: "invalid", reason: `crypto.verify threw: ${e.message}` };
  }
  return ok ? { status: "valid" } : { status: "invalid", reason: "Ed25519 manifest signature did not verify against local public.pem" };
}

// Mirrors lib/verify.js validateSkillPath().
function validateManifestSkillPath(skillPath) {
  if (typeof skillPath !== "string") throw new Error(`manifest skill.path must be a string, got ${typeof skillPath}`);
  if (skillPath.includes("\\")) throw new Error(`manifest skill.path must use forward slashes: ${JSON.stringify(skillPath)}`);
  if (!skillPath.startsWith("skills/")) throw new Error(`manifest skill.path must start with 'skills/': ${JSON.stringify(skillPath)}`);
  if (skillPath.includes("..")) throw new Error(`manifest skill.path must not contain '..': ${JSON.stringify(skillPath)}`);
  return skillPath;
}

// Buffered-tarball cap; EXCEPTD_TARBALL_SIZE_CAP_BYTES overrides it.
const TARBALL_SIZE_CAP_BYTES_DEFAULT = 200 * 1024 * 1024;
function tarballSizeCap() {
  const env = parseInt(process.env.EXCEPTD_TARBALL_SIZE_CAP_BYTES, 10);
  return Number.isFinite(env) && env > 0 ? env : TARBALL_SIZE_CAP_BYTES_DEFAULT;
}

async function main() {
  const opts = parseArgs(process.argv);
  const localPkg = JSON.parse(fs.readFileSync(path.join(ROOT, "package.json"), "utf8"));
  const localVersion = localPkg.version;

  // Refused before any network attempt, and unconditional with respect to
  // EXCEPTD_REGISTRY_FIXTURE — the fixture stubs only the /latest read, so
  // honoring it here would let the tarball fetch reach the network anyway.
  if (opts.airGap) {
    emit({
      ok: false,
      source: "air-gap",
      error: "air-gap: refresh --network requires network egress; refused. Use --from-cache --apply for the offline path.",
    }, opts.json);
    process.exitCode = 4; return;
  }

  progress(`local v${localVersion} — querying npm registry...`, opts.json);

  let meta;
  try {
    if (process.env.EXCEPTD_REGISTRY_FIXTURE) {
      // Fixture shape mirrors upstream-check's /<pkg>/latest response: version +
      // dist.tarball + dist.shasum, or version alone for the already-at-latest path.
      meta = JSON.parse(fs.readFileSync(process.env.EXCEPTD_REGISTRY_FIXTURE, "utf8"));
    } else {
      meta = await getJson(`https://registry.npmjs.org/${encodeURIComponent(PKG_NAME).replace(/%40/g, "@").replace(/%2F/g, "/")}/latest`, opts.timeoutMs);
    }
  } catch (e) {
    emit({ ok: false, error: `registry unreachable: ${e.message}`, hint: "Network required. Air-gap workflow: run `exceptd refresh --prefetch` on a connected host, then `exceptd refresh --from-cache --apply` offline. Or set EXCEPTD_REGISTRY_FIXTURE for offline testing." }, opts.json);
    process.exitCode = 2; return;
  }

  const latestVersion = meta.version;
  const tarballUrl = meta.dist && meta.dist.tarball;
  const tarballShasum = meta.dist && meta.dist.shasum;
  const tarballIntegrity = meta.dist && meta.dist.integrity; // SHA-512 SRI
  const registrySignatures = Array.isArray(meta.dist && meta.dist.signatures) ? meta.dist.signatures : [];
  if (!tarballUrl) {
    emit({ ok: false, error: "registry metadata missing dist.tarball" }, opts.json);
    process.exitCode = 2; return;
  }

  if (latestVersion === localVersion && !opts.force) {
    emit({ ok: true, message: `already at latest v${localVersion} — nothing to do. Pass --force to re-pull anyway.`, local_version: localVersion, latest_version: latestVersion, skipped: true }, opts.json);
    return;
  }

  // Probe writability before anything is fetched — failing here beats failing mid-swap.
  const writeProbe = path.join(ROOT, `.write-probe-${process.pid}`);
  try {
    fs.writeFileSync(writeProbe, "");
    fs.unlinkSync(writeProbe);
  } catch (e) {
    emit({
      ok: false,
      error: `install directory not writable: ${ROOT}`,
      hint: `Global installs typically require elevated permissions. Either: (a) run \`npm update -g @blamejs/exceptd-skills\` (recommended — same trust anchor, full package update), or (b) install locally with \`npm install @blamejs/exceptd-skills\` in a user-writable directory and retry --network there.`,
    }, opts.json);
    process.exitCode = 3; return;
  }

  // Guard the same binding the URL is built from, so the semver check is the only
  // path a metadata value reaches the fetch. A dist.tarball divergence is noted,
  // not fatal — authenticity rests on the hash and signature chain below.
  const safeVersion = typeof latestVersion === "string" ? latestVersion : "";
  if (!SEMVER_RE.test(safeVersion)) {
    emit({ ok: false, error: `registry metadata version is not valid semver: ${safeVersion.slice(0, 64)}` }, opts.json);
    process.exitCode = 2; return;
  }
  const canonicalUrl = canonicalTarballUrl(safeVersion);
  if (tarballUrl !== canonicalUrl) {
    progress(`note: registry dist.tarball (${tarballUrl}) differs from canonical npm URL; fetching ${canonicalUrl}`, opts.json);
  }

  progress(`fetching ${canonicalUrl} (${tarballShasum?.slice(0, 12) || "no shasum"})...`, opts.json);

  let tgzBuf;
  try {
    tgzBuf = await getBuffer(canonicalUrl, opts.timeoutMs);
  } catch (e) {
    emit({ ok: false, error: `tarball fetch failed: ${e.message}` }, opts.json);
    process.exitCode = 2; return;
  }

  const sizeCap = tarballSizeCap();
  if (tgzBuf.length > sizeCap) {
    emit({ ok: false, error: `tarball exceeds size cap: ${tgzBuf.length} bytes > ${sizeCap} (EXCEPTD_TARBALL_SIZE_CAP_BYTES)` }, opts.json);
    process.exitCode = 4; return;
  }

  // Both hashes come from the same metadata response as the tarball URL, so they
  // prove only that the download matches what that response declared.
  // `integrityVerified` gates the unsigned data/ + manifest-snapshot.json swap.
  let integrityVerified = false;
  if (tarballIntegrity && /^sha512-/.test(tarballIntegrity)) {
    const expected = tarballIntegrity.slice("sha512-".length);
    const actual = crypto.createHash("sha512").update(tgzBuf).digest("base64");
    if (actual !== expected) {
      emit({ ok: false, error: `tarball SHA-512 integrity mismatch: dist.integrity=${tarballIntegrity}, actual=sha512-${actual}` }, opts.json);
      process.exitCode = 4; return;
    }
    integrityVerified = true;
  } else if (tarballIntegrity) {
    // A non-sha512 SRI is not checked, so it does not count toward integrityVerified.
    progress(`note: dist.integrity present but not sha512: ${tarballIntegrity.slice(0, 40)}`, opts.json);
  }

  if (tarballShasum) {
    const actual = crypto.createHash("sha1").update(tgzBuf).digest("hex");
    if (actual !== tarballShasum) {
      emit({ ok: false, error: `tarball shasum mismatch: expected ${tarballShasum}, got ${actual}` }, opts.json);
      process.exitCode = 4; return;
    }
    integrityVerified = true;
  }

  // Without a verified transport hash, a tampered registry response — or a hostile
  // EXCEPTD_REGISTRY_FIXTURE — injects arbitrary catalog content.
  if (!integrityVerified) {
    emit({
      ok: false,
      error: "registry metadata carries no verifiable transport integrity (dist.integrity sha512 SRI and dist.shasum both absent) — refusing to swap unsigned data/ + manifest-snapshot.json",
      hint: "data/ and manifest-snapshot.json are not covered by the per-skill or manifest Ed25519 signature; a transport hash is their only integrity anchor. Run `npm update -g @blamejs/exceptd-skills` for the full provenance-verified path.",
    }, opts.json);
    process.exitCode = 4; return;
  }

  // Made with npm's registry private key, so it survives a forged metadata
  // response; absent/unverifiable falls back to the hash gates above.
  const regSig = verifyNpmRegistrySignature(PKG_NAME, latestVersion, tarballIntegrity, registrySignatures);
  if (regSig.status === "invalid" || regSig.status === "unknown-keyid") {
    emit({
      ok: false,
      error: `npm registry signature ${regSig.status} for ${PKG_NAME}@${latestVersion} — refusing to swap`,
      reason: regSig.reason || null,
      keyid: regSig.keyid || null,
      hint: "dist.signatures is present but did not verify against the pinned npm registry key. A forged metadata response cannot reproduce this signature, so treat it as tampering. Run `npm update -g @blamejs/exceptd-skills` for the full provenance-verified path, or report at https://github.com/blamejs/exceptd-skills/issues.",
    }, opts.json);
    process.exitCode = 5; return;
  }
  if (regSig.status === "absent" || regSig.status === "unverifiable") {
    progress(`note: npm registry signature ${regSig.status} (${regSig.reason || "no dist.signatures"}) — relying on transport-hash integrity only`, opts.json);
  }

  let tarBuf;
  try { tarBuf = zlib.gunzipSync(tgzBuf); }
  catch (e) { emit({ ok: false, error: `gunzip: ${e.message}` }, opts.json); process.exitCode = 4; return; }

  const entries = parseTar(tarBuf);
  // npm tarballs prefix every entry with "package/"
  const stripPkg = (n) => n.startsWith("package/") ? n.slice("package/".length) : n;

  const tarballManifestEntry = entries.find((e) => stripPkg(e.name) === "manifest.json");
  const tarballPubKeyEntry   = entries.find((e) => stripPkg(e.name) === "keys/public.pem");
  if (!tarballManifestEntry) {
    emit({ ok: false, error: "tarball missing manifest.json" }, opts.json);
    process.exitCode = 4; return;
  }

  // The trust anchor is the LOCAL public.pem; a differing tarball fingerprint is
  // a rotation, which requires a full `npm update -g`.
  const localPubKeyPath = path.join(ROOT, "keys", "public.pem");
  let localPubKeyText, tarballPubKeyText;
  try { localPubKeyText = fs.readFileSync(localPubKeyPath, "utf8"); }
  catch (e) { emit({ ok: false, error: `local keys/public.pem unreadable: ${e.message}` }, opts.json); process.exitCode = 4; return; }
  if (tarballPubKeyEntry) tarballPubKeyText = tarballPubKeyEntry.body.toString("utf8");

  const localFp = fingerprintPublicKey(localPubKeyText);
  const tarballFp = tarballPubKeyText ? fingerprintPublicKey(tarballPubKeyText) : null;
  if (tarballFp && tarballFp !== localFp) {
    emit({
      ok: false,
      error: `public key fingerprint mismatch: local=${localFp} tarball=${tarballFp}`,
      hint: `The maintainer rotated the Ed25519 signing key in v${latestVersion}. Key rotations require an explicit \`npm update -g @blamejs/exceptd-skills\` so you can audit the trust transition. Refusing to swap on --network.`,
    }, opts.json);
    process.exitCode = 5; return;
  }

  // local ↔ tarball agreement still passes for an attacker who swapped both, so
  // keys/EXPECTED_FINGERPRINT is the external anchor. KEYS_ROTATED=1 bypasses it.
  const expectedFingerprintPath = path.join(ROOT, "keys", "EXPECTED_FINGERPRINT");
  if (fs.existsSync(expectedFingerprintPath) && process.env.KEYS_ROTATED === "1") {
    process.emitWarning(
      `EXPECTED_FINGERPRINT pin check skipped via KEYS_ROTATED=1 during refresh-network. ` +
      `Update keys/EXPECTED_FINGERPRINT to lock the new pin once rotation completes.`,
      { code: 'EXCEPTD_KEYS_ROTATED_OVERRIDE' }
    );
  }
  if (fs.existsSync(expectedFingerprintPath) && !process.env.KEYS_ROTATED) {
    // The shared loader tolerates a BOM-prefixed pin file; an inline split-trim
    // keeps the BOM on the first line and blocks every legitimate run. Only
    // ENOENT/EACCES is swallowed — any other error must refuse.
    let expectedFp;
    let loaderFailedSoftly = false;
    try {
      const { loadExpectedFingerprintFirstLine } = require("./verify.js");
      expectedFp = loadExpectedFingerprintFirstLine(expectedFingerprintPath);
    } catch (err) {
      if (err && (err.code === "ENOENT" || err.code === "EACCES")) {
        // Pin file deleted under us, or not readable by this uid.
        loaderFailedSoftly = true;
      } else {
        emit({
          ok: false,
          error: `keys/EXPECTED_FINGERPRINT loader threw unexpectedly: ${err && err.message}`,
          pin_path: expectedFingerprintPath,
          hint: "The pin file exists but the loader hit a non-IO error. Refusing to swap on --network; investigate the loader failure before retrying.",
        }, opts.json);
        process.exitCode = 5; return;
      }
    }
    if (!loaderFailedSoftly) {
      // The loader returns null for a pin file it cannot decode; the operator
      // placed a pin, so an unreadable one is a hard fail, not an unpinned refresh.
      if (expectedFp === null || expectedFp === undefined) {
        emit({
          ok: false,
          error: `keys/EXPECTED_FINGERPRINT exists but its bytes could not be parsed (likely UTF-16 BOM or non-UTF-8 encoding)`,
          pin_path: expectedFingerprintPath,
          hint: "Re-save the pin file as UTF-8 (the loader accepts UTF-8-with-BOM). Refusing to swap on --network with an unparseable pin.",
        }, opts.json);
        process.exitCode = 5; return;
      }
      // The pin file is canonically `SHA256:<base64>` while fingerprintPublicKey()
      // returns bare base64, so the prefix comes off before the compare.
      const expectedFpBase64 = expectedFp.startsWith("SHA256:")
        ? expectedFp.slice("SHA256:".length)
        : expectedFp;
      if (expectedFpBase64 !== localFp) {
        emit({
          ok: false,
          error: `local keys/public.pem fingerprint diverges from keys/EXPECTED_FINGERPRINT pin`,
          local_fingerprint: "SHA256:" + localFp,
          pinned_fingerprint: expectedFp,
          hint: "Either keys/public.pem was rotated since the pin was set (rerun `npm run bootstrap` to re-pin), or the local public.pem was tampered with. Set KEYS_ROTATED=1 to bypass once. Refusing to swap on --network.",
        }, opts.json);
        process.exitCode = 5; return;
      }
    }
  }

  let tarballManifest;
  try { tarballManifest = JSON.parse(tarballManifestEntry.body.toString("utf8")); }
  catch (e) { emit({ ok: false, error: `tarball manifest.json parse: ${e.message}` }, opts.json); process.exitCode = 4; return; }

  // Authenticate the manifest envelope against the LOCAL public key before
  // honoring any entry in it: per-skill signatures cover only skill body bytes,
  // so paths and names around them could be re-shaped without breaking one. A
  // missing signature refuses here rather than warning.
  const manifestSigResult = verifyTarballManifestSignature(tarballManifest, localPubKeyText);
  if (manifestSigResult.status !== "valid") {
    emit({
      ok: false,
      error: `tarball manifest_signature ${manifestSigResult.status} — refusing to swap`,
      reason: manifestSigResult.reason || null,
      hint: manifestSigResult.status === "missing"
        ? "Tarball predates v0.12.17 manifest signing. Run `npm update -g @blamejs/exceptd-skills` instead so the full provenance-verified install path runs."
        : "Tarball manifest envelope failed Ed25519 verification against the LOCAL public key. Run `npm update -g @blamejs/exceptd-skills` for the full provenance-verified path, or report this tarball at https://github.com/blamejs/exceptd-skills/issues.",
    }, opts.json);
    process.exitCode = 5; return;
  }

  // `path` is the payload's forward-slash location in the tarball, never a name
  // synthesized from the entry's id, and is validated before it resolves against
  // the extracted tree, so a tampered manifest cannot traverse out of it.
  const localKeyObj = crypto.createPublicKey(localPubKeyText);
  const skills = Array.isArray(tarballManifest.skills) ? tarballManifest.skills : [];
  const failures = [];
  let verifiedCount = 0;
  for (const sk of skills) {
    if (!sk || typeof sk.name !== "string" || typeof sk.signature !== "string") {
      failures.push({ name: sk?.name || "(missing name)", reason: "manifest entry missing name or signature" });
      continue;
    }
    let normalizedPath;
    try { normalizedPath = validateManifestSkillPath(sk.path); }
    catch (e) { failures.push({ name: sk.name, reason: `manifest path rejected: ${e.message}` }); continue; }
    const payloadEntry = entries.find((e) => stripPkg(e.name) === normalizedPath);
    if (!payloadEntry) { failures.push({ name: sk.name, reason: `payload missing from tarball: ${normalizedPath}` }); continue; }
    const normalized = normalizeSkillBytes(payloadEntry.body);
    const ok = verifyDetached(localKeyObj, normalized, sk.signature);
    if (ok) verifiedCount++;
    else failures.push({ name: sk.name, reason: "Ed25519 signature did not verify against local public key" });
  }

  if (skills.length === 0) {
    emit({
      ok: false,
      error: "tarball manifest.json declares zero skills — refusing to swap",
      verified: 0,
      total: 0,
      hint: "A legitimate tarball must declare at least one skill. Treat this as a tarball-integrity failure.",
    }, opts.json);
    process.exitCode = 5; return;
  }
  if (verifiedCount !== skills.length || failures.length > 0) {
    emit({
      ok: false,
      error: `${failures.length}/${skills.length} skill signature(s) failed verification — refusing to swap`,
      failures: failures.slice(0, 10),
      verified: verifiedCount,
      total: skills.length,
      hint: "Refusing to install unverified content. Run `npm update -g @blamejs/exceptd-skills` for the full provenance-verified path, or report this tarball at https://github.com/blamejs/exceptd-skills/issues.",
    }, opts.json);
    process.exitCode = 5; return;
  }

  // A skill payload the manifest does not declare refuses the swap, so a
  // tarball that adds one outside the manifest cannot slip past the loop above.
  const manifestSkillPaths = new Set(skills.map(s => validateManifestSkillPath(s.path)));
  const tarballSkillPayloads = entries
    .map(e => stripPkg(e.name))
    .filter(name => /^skills\/[^/]+\/skill\.md$/.test(name));
  for (const tp of tarballSkillPayloads) {
    if (!manifestSkillPaths.has(tp)) {
      emit({
        ok: false,
        error: `tarball ships skill payload not declared in manifest: ${tp} — refusing to swap`,
        hint: "Tarball+manifest divergence. Report at https://github.com/blamejs/exceptd-skills/issues.",
      }, opts.json);
      process.exitCode = 5; return;
    }
  }

  if (opts.dryRun) {
    emit({
      ok: true,
      dry_run: true,
      local_version: localVersion,
      latest_version: latestVersion,
      verified_skills: verifiedCount,
      total_skills: skills.length,
      message: `--dry-run: would swap data/ + manifest.json from v${latestVersion} (${verifiedCount}/${skills.length} signatures verified). No files changed.`,
    }, opts.json);
    return;
  }

  // Every old target moves into one backup dir before any new target moves in;
  // a failure walks completedSteps in reverse to restore.
  const stageDir = fs.mkdtempSync(path.join(ROOT, ".refresh-network-"));
  let written = 0;
  let backupDir = null;
  const completedSteps = []; // [{kind: 'backup' | 'install', target}]
  try {
    for (const entry of entries) {
      const rel = stripPkg(entry.name);
      // A data refresh: bin/, lib/ and package.json are left alone.
      if (!(rel === "manifest.json" || rel === "manifest-snapshot.json" ||
            rel.startsWith("data/") || rel.startsWith("skills/"))) continue;
      const dst = path.join(stageDir, rel);
      fs.mkdirSync(path.dirname(dst), { recursive: true });
      fs.writeFileSync(dst, entry.body);
      written++;
    }

    // PID plus random suffix: concurrent invocations collide on the ms clock alone.
    const backupSuffix = `${process.pid}-${crypto.randomBytes(4).toString("hex")}`;
    backupDir = path.join(ROOT, `.refresh-network-backup-${Date.now()}-${backupSuffix}`);
    fs.mkdirSync(backupDir);
    const replaceList = ["data", "skills", "manifest.json", "manifest-snapshot.json"];

    // Phase A: the install root holds none of the replaced targets after this.
    for (const target of replaceList) {
      const dst = path.join(ROOT, target);
      if (fs.existsSync(dst)) {
        fs.renameSync(dst, path.join(backupDir, target));
        completedSteps.push({ kind: "backup", target });
      }
    }

    // Phase B: move all new targets in from stage.
    for (const target of replaceList) {
      const src = path.join(stageDir, target);
      if (!fs.existsSync(src)) continue;
      fs.renameSync(src, path.join(ROOT, target));
      completedSteps.push({ kind: "install", target });
    }

    fs.rmSync(stageDir, { recursive: true, force: true });
    // The backup dir stays on disk so an operator can roll back by hand.
    emit({
      ok: true,
      local_version: localVersion,
      latest_version: latestVersion,
      verified_skills: verifiedCount,
      total_skills: skills.length,
      files_written: written,
      backup_dir: path.relative(ROOT, backupDir),
      registry_signatures_present: registrySignatures.length,
      registry_signature_status: regSig.status,
      registry_signature_keyid: regSig.keyid || null,
      message: `refreshed catalog from v${localVersion} → v${latestVersion} (${verifiedCount}/${skills.length} skill signatures verified, npm registry signature: ${regSig.status}). Backup at ${path.relative(ROOT, backupDir)} — safe to remove after verifying the new run.`,
    }, opts.json);
  } catch (e) {
    const rollbackErrors = [];
    for (const step of [...completedSteps].reverse()) {
      try {
        if (step.kind === "install") {
          fs.rmSync(path.join(ROOT, step.target), { recursive: true, force: true });
        } else if (step.kind === "backup" && backupDir) {
          const src = path.join(backupDir, step.target);
          const dst = path.join(ROOT, step.target);
          if (fs.existsSync(src)) fs.renameSync(src, dst);
        }
      } catch (re) {
        rollbackErrors.push({ target: step.target, kind: step.kind, error: re.message });
      }
    }
    fs.rmSync(stageDir, { recursive: true, force: true });
    emit({
      ok: false,
      error: `swap failed mid-rename: ${e.message}`,
      rolled_back: rollbackErrors.length === 0,
      rollback_errors: rollbackErrors,
      backup_dir: backupDir ? path.relative(ROOT, backupDir) : null,
      hint: rollbackErrors.length === 0
        ? "Auto-rollback completed. Install state matches pre-refresh. Re-run `exceptd refresh --network` or `npm install -g @blamejs/exceptd-skills` to retry."
        : "Auto-rollback partially failed. Restore manually from the backup dir at the install root, or reinstall with `npm install -g @blamejs/exceptd-skills`.",
    }, opts.json);
    process.exitCode = 4;
  }
}

if (require.main === module) {
  main().catch((err) => {
    process.stderr.write(`refresh-network: fatal: ${err && err.message || err}\n`);
    process.exit(2);
  });
}

module.exports = {
  parseTar,
  fingerprintPublicKey,
  normalizeSkillBytes,
  verifyTarballManifestSignature,
  canonicalManifestBytesForRefresh,
  isAllowedTarballHost,
  verifyNpmRegistrySignature,
  NPM_REGISTRY_KEYS,
};
