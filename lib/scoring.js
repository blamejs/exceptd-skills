...
function deriveRwepFromFactors(factors) {
  ...
  // active_exploitation's string-ladder form legitimately appears in BOTH
  // shapes: Shape A stores it as the literal ladder string, and a Shape B
  // (post-weight) catalog block can ALSO carry it as a string alongside its
  // post-weight integers (the integer is the active_exploitation contribution;
  // the string is the human-readable status). So a ladder STRING is not
  // Shape-A evidence on its own — only a non-active_exploitation key holding a
  // boolean/ladder-string is. Treating a string active_exploitation as Shape-A
  // misrouted a Shape-B block into scoreCustom, which does not read the
  // post-weight ai_factor key and silently dropped the +15 AI weight.
  const aeAllowed = new Set(['none', 'unknown', 'suspected', 'theoretical', 'confirmed']);
  const hasBooleanOrLadder = entries.some(
    ([k, v]) => k !== 'active_exploitation' &&
      (typeof v === 'boolean' || (typeof v === 'string' && aeAllowed.has(v.trim().toLowerCase()))),
  );
