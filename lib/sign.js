...
function generateKeypair({ rotate = false } = {}) {
  ...
  if (rotate) {
    console.log('[sign] Keypair rotated. All existing signatures are now invalid — run sign-all to re-sign.');
  } else {
  ...
  console.log('\nNext steps:');
  console.log('  1. exceptd doctor --fix     — chains sign-all after key generation');
