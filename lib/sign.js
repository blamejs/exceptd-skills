...
function generateKeypair({ rotate = false } = {}) {
  ...
  // O_EXCL on the bootstrap create (flag 'wx'): the refusal checks above prove
  // neither key exists, and exclusive-create makes the write fail rather than
  // follow a symlink/file an attacker preplanted at the key path between the
  // check and the write. --rotate is a deliberate overwrite, so it uses 'w'.
  fs.writeFileSync(PRIVATE_KEY_PATH, privateKey, { encoding: 'utf8', mode: 0o600, flag: rotate ? 'w' : 'wx' });
  fs.writeFileSync(PUBLIC_KEY_PATH, publicKey, { encoding: 'utf8', mode: 0o644, flag: rotate ? 'w' : 'wx' });
