...
const FEEDS = [
  ...
  // close (b); the nightmare-eclipse-gitlab feed closes (a) as the
...
async function fetchFeed(feed, ctx) {
  ...
  // Adopt the vendored blamejs retry primitive (BJS-02): a transient feed
  // failure (HTTP 429/5xx, timeout/abort, network reset) is retried with
  // backoff + jitter instead of dropping the feed on the first hiccup. A
  // permanent 4xx is not retried. Each attempt gets a fresh abort timer.
  const attempt = async () => {
...
const ADVISORIES_SOURCE = {
  ...
  // The no-op applyDiff below already refuses to write, but the flag is what
  // the *callers* read. refresh-external only counts a source toward the
  // upsert total when `!report_only`, and the refresh workflow gates its apply
  // step and its entire open-pr job on that total being non-zero. This feed
  // reports every CVE seen across USN / RHSA / ZDI / CISA that the catalog
  // does not track — structurally in the thousands, and never zero, because
  // the catalog deliberately does not carry every Ubuntu kernel CVE. Leaving
  // the flag off therefore pinned the upsert count permanently above zero and
  // made "open a PR only when something changed" unconditional.
  report_only: true,
  async fetchDiff(ctx) {
