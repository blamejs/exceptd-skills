...
// Production OSV endpoint. Requests land here over HTTPS unless a test
// harness sets OSV_HOST_OVERRIDE, whose parsing and host/port/transport
// substitution are documented at osvTransport() below.
const OSV_HOST = "api.osv.dev";
const REQUEST_TIMEOUT_MS = 10000;
const USER_AGENT = "exceptd-security/source-osv (+https://exceptd.com)";

// Cap the streamed response so a hostile or misconfigured endpoint (a hijacked
// DNS / compromised CDN / OSV_HOST_OVERRIDE pointed at a malicious server)
// can't stream unbounded bytes into RAM. A real /v1/vulns/{id} or /v1/query
// response is a few KB; 16 MB is generous headroom. Symmetric with the JSON
// cap getJsonOnce enforces in lib/refresh-network.js. Tunable via
// EXCEPTD_OSV_RESPONSE_CAP_BYTES for future API growth.
...
function osvRequestOnce({ method, reqPath, body, timeoutMs }) {
  return new Promise((resolve, reject) => {
    ...
    const req = mod.request(opts, (res) => {
      ...
      res.on("end", () => {
        // If the cap fired, req.destroy already routed through req.on("error");
        // don't also resolve here (avoids a double-settle on the promise).
        if (capped) return;
        try {
