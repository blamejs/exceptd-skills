...
function checkSourceHashes(liveSources, recorded, absFn) {
  ...
  for (const p of liveSources) {
    ...
    // descriptor — no path stat-then-read, so there is no check-then-use window.
    // O_NOFOLLOW refuses a symlinked leaf at open (ELOOP) on POSIX; an absent
    // file (ENOENT) is reported missing instead of crashing; the fstat type
    // check refuses a dir/junction/fifo on every platform.
    let live;
    {
      let fd;
