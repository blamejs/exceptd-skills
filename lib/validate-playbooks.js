...
function checkCrossRefs(playbook, ctx, playbookIds) {
  ...
  const conditionParses = (cond) => {
    ...
    const runErrors = [];
    try { _evalCondition(cond, { _runErrors: runErrors }, { _runErrors: runErrors }); }
    catch { return true; } // an evaluator throw is a separate concern, not "unparseable"
    return !runErrors.some((e) => e && e.kind === 'condition_unparsed');
  };
  for (const [i, ec] of (((phases.analyze || {}).escalation_criteria) || []).entries()) {
