...
function verifyManifestSignature(manifest) {
  ...
  if (pinResult.status === 'no-pin' && !pinResult.rotationOverride) {
    return {
      status: 'invalid',
      reason: `key-pin absent: keys/EXPECTED_FINGERPRINT is missing, so a swapped keys/public.pem cannot be detected. The pin is shipped and committed; restore it, or set KEYS_ROTATED=1 for an intentional rotation.`,
      pin_absent: true,
    };
  }
  let signatureBytes;
