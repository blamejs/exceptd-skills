...
// ===========================================================================
// refresh-discovery-cache-integrity — discoverNewKev sidecar integrity
//
// Auto-discovery reads per-CVE NVD/EPSS cache files and writes their CVSS / CWE
// / EPSS values into auto-imported catalog drafts. Those reads must be
// integrity-checked against the signed _index.json the rest of the cache consume
// path verifies — otherwise an attacker who drops a forged sidecar for a
// newly-DISCOVERED CVE between prefetch and refresh gets forged mechanical
// fields written with zero verification. Discovery cannot throw (it returns
// drafts), so the fail-closed behavior is to drop the unverified payload.
// ===========================================================================

test_describe('refresh-discovery-cache-integrity', () => {
  const crypto = require('node:crypto');

  const DISC_ROOT = path.join(__dirname, '..');
  const { discoverNewKev: discoverNewKevDisc } = require(path.join(DISC_ROOT, 'lib', 'auto-discovery.js'));
  ...
  test('discovered draft does NOT absorb a tampered NVD sidecar (sha256 mismatch)', () => {
    ...
    try {
      ...
      const r = discoverNewKevDisc(ctx);
...
// ---- merged from tests/nightly-enrich.test.js ----
// Nightly prefetch also fetches NVD/EPSS for newly-KEV-listed CVE ids, so a
// nightly KEV draft (buildKevDraftEntry) arrives pre-enriched instead of with
// null mechanical fields.
const { describe } = require('node:test');
describe('nightly-enrich (buildKevDraftEntry mechanical-field enrichment + DEFAULT_CAP)', () => {
