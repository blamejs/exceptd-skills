...
/**
 * Subject suite for the `exceptd currency` orchestrator-passthrough verb.
 *
 * A --json success carries top-level ok:true + currency_report[]; an unknown
 * flag is rejected (exit 1). currency emits a scheduler log line before the
 * envelope, so the JSON envelope is the LAST stdout line.
 */
