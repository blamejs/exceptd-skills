...
/**
 * tests/mail-server-hardening.test.js
 *
 * Structural + cross-reference pin for the mail-server-hardening playbook
 * (data/playbooks/mail-server-hardening.json).
 *
 * Asserts:
 ...
 *   - The declared CVE references (SMTP-smuggling + STARTTLS-injection) resolve
 *     against the live CVE catalog — they are the playbook's reason for being.
 *   - feeds_into / remediation for_signals / fp_profile cross-refs resolve.
 *   - The mail-specific surface (SMTP smuggling, STARTTLS buffer drain, open
 *     relay indicators) is present.
 */
...
const PLAYBOOK_ID = "mail-server-hardening";
...
test("declared CVE references resolve against the live CVE catalog", () => {
  ...
  assert.ok(cveRefs.length >= 1, "mail-server-hardening declares CVE references");
  // SMTP smuggling + STARTTLS injection are this playbook's anchor CVEs.
  assert.ok(cveRefs.includes("CVE-2023-51764"), "the SMTP-smuggling CVE must be referenced");
...
test("mail-protocol surface: smuggling, STARTTLS-drain, and open-relay indicators are present", () => {
  const pb = loadPlaybook();
  const indIds = new Set(pb.phases.detect.indicators.map((i) => i.id));
  assert.ok(indIds.has("smtp-smuggling-end-of-data-accepted"), "the SMTP-smuggling indicator must be present");
  assert.ok(indIds.has("starttls-receive-buffer-not-drained"), "the STARTTLS-buffer-drain indicator must be present");
  assert.ok(indIds.has("inbound-open-relay"), "the open-relay indicator must be present");
