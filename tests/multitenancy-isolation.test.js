 ...
 *
 * Structural + cross-reference pin for the multitenancy-isolation playbook
 * (data/playbooks/multitenancy-isolation.json).
 *
 * Asserts:
 ...
 *   - The Rapid-Reset CVE (CVE-2023-44487) resolves against the live catalog.
 *   - feeds_into / remediation for_signals / fp_profile cross-refs resolve.
 *   - The two linked surfaces (cross-tenant isolation + DoS resilience) are
 *     both represented in the indicator set.
 *   - rwep_inputs reference real indicator ids.
 */
...
const PLAYBOOK_ID = "multitenancy-isolation";
...
test("the Rapid-Reset CVE reference resolves against the live CVE catalog", () => {
  const pb = loadPlaybook();
  const cveRefs = pb.domain.cve_refs || [];
  assert.ok(cveRefs.includes("CVE-2023-44487"), "the HTTP/2 Rapid Reset CVE must be referenced");
...
test("remediation for_signals, fp_profile, and rwep_inputs all reference real indicator ids", () => {
  const pb = loadPlaybook();
  ...
  for (const ri of pb.phases.analyze.rwep_inputs || []) {
    assert.ok(indIds.has(ri.signal_id), `rwep_inputs signal_id "${ri.signal_id}" must reference a real indicator`);
  }
});

test("both linked surfaces — cross-tenant isolation and DoS resilience — are represented", () => {
  const pb = loadPlaybook();
  const indIds = new Set(pb.phases.detect.indicators.map((i) => i.id));
  // Isolation dimension.
  assert.ok(indIds.has("tenant-id-trusted-from-client"), "the client-trusted tenant-id isolation indicator must be present");
  assert.ok(indIds.has("query-not-scoped-by-tenant"), "the unscoped-query isolation indicator must be present");
  // Availability / DoS dimension.
  assert.ok(indIds.has("http2-rapid-reset-uncapped"), "the Rapid-Reset availability indicator must be present");
