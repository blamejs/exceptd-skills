...
/**
 * tests/network-trust.test.js
 *
 * Structural + cross-reference pin for the network-trust playbook
 * (data/playbooks/network-trust.json).
 *
 * Asserts:
 ...
 *   - The DNSSEC resolver-DoS CVEs (KeyTrap + NSEC3) resolve against the live
 *     catalog.
 *   - jurisdiction_obligations carry a closed-vocabulary clock_starts and the
 *     close.notification_actions obligation_refs resolve back to them.
 *   - feeds_into / remediation for_signals / fp_profile cross-refs resolve.
 *   - The AiTM trust-anchor surface (DNSSEC validation, DANE/TLSA, authenticated
 *     time) is represented in the indicator set.
 */
...
const PLAYBOOK_ID = "network-trust";
...
test("the DNSSEC resolver-DoS CVEs (KeyTrap + NSEC3) resolve against the live CVE catalog", () => {
  const pb = loadPlaybook();
  const cveRefs = pb.domain.cve_refs || [];
  assert.ok(cveRefs.includes("CVE-2023-50387"), "the KeyTrap CVE must be referenced");
  assert.ok(cveRefs.includes("CVE-2023-50868"), "the NSEC3 CVE must be referenced");
...
test("notification_actions obligation_refs resolve back to a jurisdiction obligation", () => {
  const pb = loadPlaybook();
  const obligations = pb.phases.govern.jurisdiction_obligations || [];
  assert.ok(obligations.length >= 1, "expected at least one jurisdiction obligation");
  // Each obligation's clock_starts must be the closed-vocab "detect_confirmed".
  for (const o of obligations) {
    assert.equal(typeof o.clock_starts, "string");
    assert.ok(o.clock_starts.length > 0);
  }
  const obligationKeys = new Set(obligations.map(obligationKey));
  const actions = (pb.phases.close && pb.phases.close.notification_actions) || [];
  assert.ok(actions.length >= 1, "expected at least one close notification action");
  for (const na of actions) {
    assert.ok(
      obligationKeys.has(na.obligation_ref),
      `notification obligation_ref "${na.obligation_ref}" must resolve to a govern jurisdiction_obligation key`,
    );
  }
});
...
test("AiTM trust-anchor surface: DNSSEC, DANE/TLSA, and authenticated-time indicators are present", () => {
  const pb = loadPlaybook();
  const indIds = new Set(pb.phases.detect.indicators.map((i) => i.id));
  assert.ok(indIds.has("dnssec-validation-not-enforced"), "the DNSSEC-validation indicator must be present");
  assert.ok(indIds.has("dane-tlsa-not-checked"), "the DANE/TLSA indicator must be present");
  assert.ok(indIds.has("unauthenticated-ntp-no-nts"), "the authenticated-time indicator must be present");
