...
// ---- merged from tests/prefetch-newkev.test.js ----
// Nightly prefetch also fetches NVD/EPSS for newly-KEV-listed CVE ids, so a
// nightly KEV draft (lib/auto-discovery.js buildKevDraftEntry) arrives
// pre-enriched instead of with null mechanical fields.
//
// Covers:
//   - `newKevIds(kevFeed, cveCatalog)` — the pure helper (KEV feed minus
//     local catalog), thoroughly, including edge cases.
...
describe('prefetch-newkev (newKevIds + SOURCES expand + end-to-end new-KEV prefetch)', () => {
  const fs = require('node:fs');
  const path = require('node:path');
  const os = require('node:os');
